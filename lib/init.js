...
const {
  loadFlowSpecConfig,
  ensureFlowSpecProjectConfig,
  DEFAULT_LOCALE,
  normalizeLocale,
} = require("./flowSpecConfig");
...
function ensureFlowSpecDirs(cwd) {
...
function removeFlowSpecUpdateCheckCache(cwd) {
...
function copyFlowSpecTemplates(cwd, templatesDir, options = {}) {
...
function readPackageName(templatesDir) {
  try {
    ...
    return "@double-codeing/flow-spec";
  }
}
...
function writeHookScriptWithPackageName(destDir, templatesDir, scriptName) {
  ...
  body = body.replace(/__FLOW_SPEC_PACKAGE_NAME__/g, readPackageName(templatesDir));
...
function mergeCodexUpdateCheckHook(existing) {
  ...
  next.hooks.SessionStart.push({
    ...
    hooks: [
      {
        type: "command",
        command: "node .codex/hooks/fs-update-check.js",
        statusMessage: "Checking flow-spec knowledge version",
      },
    ],
  });
...
function mergeCodexConfigSessionHook(existing) {
  ...
  next.hooks.SessionStart.unshift({
    ...
    hooks: [
      {
        type: "command",
        command: "node .codex/hooks/fs-config-session.js",
      },
    ],
  });
  return { config: next, changed: true };
...
function mergeCursorUpdateCheckHook(existing) {
  ...
  const exists = next.hooks.sessionStart.some((hook) =>
    hook && String(hook.command || "").includes("fs-update-check"),
  );
  if (exists) return { config: next, changed: false };
  next.hooks.sessionStart.push({
    command: "node .cursor/hooks/fs-update-check.js",
    timeout: 10,
  });
  return { config: next, changed: true };
}
...
function buildMergedRouting(templateRouting, existingRouting, pkgVersion, isFirstInit = false) {
  ...
  // projectRev：本项目已基线对齐到的包模板修订号（由 fs-kb-upgrade 完整流程跑完 3a/3b 后写入）。
  // init 行为：
  //   - 首次初始化（项目侧 manifest-routing.json 此前不存在）：按模板写入，等同首次落地即视为已对齐。
  //   - 已存在 manifest-routing.json：不写、不覆盖项目侧值；由 fs-kb-upgrade 在完整流程末尾改写。
  if (isFirstInit) {
  ...
  // pkgRev：本次 init 用的包模板修订号（= 包侧 projectRev 快照），供 fs-kb-upgrade 步骤 2c 取用。
...
/**
 * 把「本次 init 用的包模板 projectRev」写入项目侧 `.flow-spec/manifest-routing.json` 的 `pkgRev` 顶层字段，
 * 供 `fs-kb-upgrade` 步骤 2c 取用。在 reset 与 incremental 两条路径之后无条件调用一次。
 *
...
function upgradeFlowSpecRoutingAndMatchers(cwd, templatesDir, options = {}) {
  const { overwrite = false } = options;
  if (overwrite) {
    return {
      upgraded: false,
...
function validateFlowSpecRouting(cwd) {
  const routingPath = path.join(cwd, KNOWLEDGE_ROOT, "manifest-routing.json");
  const matchersPath = path.join(cwd, KNOWLEDGE_ROOT, "manifest-matchers.json");
...
/**
 * 将当前 locale 包模板 knowledge/index.md 原样复制到目标 cwd 下 .flow-spec/template/index.template.md，
 * 供 fs-kb-upgrade 技能步骤 3b 与宿主仓 .flow-spec/index.md 对照；init 不修改 index.md 正文。
 * 注意：模板正文声明「.flow-spec」指宿主仓；与 flow-spec 开发仓根 .flow-spec（产品自用知识库）职责不同。
 */
function copyFlowSpecIndexTemplateSnapshot(cwd, templatesDir) {
...
function copySkills(cwd, agentRoot, templatesDir) {
  ...
  if (fs.existsSync(skillsSrc)) {
    ...
    // 删除配置根中以 fs- 开头、但已不存在于当前 locale templates/skills/ 的旧 skill 目录
    // 只清理 flow-spec 管理的 skill，不触碰用户自定义 skill
    // LEGACY_SKILLS：非 fs- 开头的历史旧名，也需一并清理
    const LEGACY_SKILLS = new Set(["stock-docs-vs-req-docs"]);
    if (fs.existsSync(skillsDest)) {
      for (const name of fs.readdirSync(skillsDest)) {
        if ((name.startsWith("fs-") || LEGACY_SKILLS.has(name)) && !templateNames.has(name)) {
...
/**
 * 将包模板 rules 镜像到 `.codex/fs-rules/*.md`。
 *
 * 不用 `.codex/rules`：Codex 官方将该目录用于 exec-policy（`*.rules` / prefix_rule 命令审批）。
 * 不用 `.codex/topics`：与知识库 `.flow-spec/topics` 重名，易混淆。
 */
function writeCodexRuleMirrors(cwd, templatesDir) {
  const rulesDir = path.join(templatesDir, "rules");
  const outDir = path.join(cwd, ".codex", "fs-rules");
  ensureDir(outDir);

  // 历史目录迁移（不阻断 init）
  for (const legacyName of ["topics", "rules"]) {
    const legacyDir = path.join(cwd, ".codex", legacyName);
    if (!fs.existsSync(legacyDir)) continue;
    try {
      for (const name of fs.readdirSync(legacyDir)) {
        const src = path.join(legacyDir, name);
        if (!fs.statSync(src).isFile()) continue;
        // 官方沙箱审批规则是 *.rules，保留在 .codex/rules，不迁走
        if (legacyName === "rules" && /\.rules$/i.test(name)) continue;
        const dest = path.join(outDir, name);
        if (!fs.existsSync(dest)) {
          fs.copyFileSync(src, dest);
        }
        // 已成功镜像的旧 md 可删，避免与官方 rules 目录混放
        if (fs.existsSync(dest)) {
          fs.rmSync(src, { force: true });
        }
      }
      const remain = fs.existsSync(legacyDir) ? fs.readdirSync(legacyDir) : [];
      if (remain.length === 0) {
  ...
  // 与 Cursor/Claude 的 rules 模板同源；落盘到 fs-rules，避免占用官方 .codex/rules
  const names = fs
...
function writeCodexEntry(cwd, templatesDir, projectConfig) {
  ...
  writeCodexRuleMirrors(cwd, templatesDir);
}
 ...
 * @param {boolean} [options.overwriteFlowSpec]
...
async function run(cwd, agentIds, options = {}) {
  const { overwriteFlowSpec = false, configValues } = options;
  const ids = normalizeAgentIds(agentIds || []);
  const templatesRoot = path.join(__dirname, "..", "templates");
  const existingConfig = loadFlowSpecConfig(cwd);
  ...
  ensureFlowSpecDirs(cwd);
  removeFlowSpecUpdateCheckCache(cwd);
  ensureFlowSpecProjectConfig(cwd, templatesDir, {
    overwrite: false,
    values: Object.keys(effectiveConfigValues).length ? effectiveConfigValues : undefined,
  });
  const knowledgeResult = copyFlowSpecTemplates(cwd, templatesDir, {
    overwrite: overwriteFlowSpec,
  });
  const routingUpgrade = upgradeFlowSpecRoutingAndMatchers(cwd, templatesDir, {
    overwrite: overwriteFlowSpec,
  });
  finalizePkgRev(cwd, templatesDir);
  validateFlowSpecRouting(cwd);

  const indexSnapshot = copyFlowSpecIndexTemplateSnapshot(cwd, templatesDir);

  const projectConfig = loadFlowSpecConfig(cwd);
