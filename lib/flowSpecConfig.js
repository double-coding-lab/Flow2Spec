...
const CONFIG_FILENAME = "flow-spec.config.json";
...
const CONFIG_FIELDS = [
  ...
  },
  {
    ...
    type: "boolean",
    default: true,
    question: "启用意图识别自动分流（高置信操作意图自动进入对应 fs-* 技能，默认 Y）？",
  },
  {
    key: "changeTracking.feat",
    type: "boolean",
    default: true,
    question: "启用变更追踪 - fs-kb-feat（新增能力时创建可续作的任务清单）？",
  },
  {
    key: "changeTracking.fix",
    type: "boolean",
    default: false,
    question: "启用变更追踪 - fs-kb-fix（修正能力时创建可续作的任务清单）？",
  },
  {
    key: "changeTracking.implement",
    type: "boolean",
    default: true,
    question: "启用变更追踪 - fs-implement-tech-design（实现技术方案时创建可续作的任务清单）？",
  },
  {
    key: "updateCheck.enabled",
    type: "boolean",
    default: true,
    question: "启用每日版本更新提示（每天第一次 Agent 对话时检查是否有新版 flow-spec）？",
  },
  {
...
/**
 * 读取项目根 flow-spec.config.json，与 DEFAULTS 合并。
 * 文件不存在时返回默认副本（不自动创建文件）。
 * changeTracking 兼容旧版布尔值（true/false → 全部子项同值）。
 */
function loadFlowSpecConfig(cwd) {
 ...
 */
function ensureFlowSpecProjectConfig(cwd, templatesDir, options = {}) {
