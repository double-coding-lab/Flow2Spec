...
> 长文见配置根 **`skills/fs-git-commit/SKILL.md`**；本仓模板源见 **`templates/skills/fs-git-commit/SKILL.md`**。
...
- 用户触发 `fs-git-commit`、`git commit`、提交代码、帮我提交。
- 用户明确说“快捷提交”“快速提交”或 “quick commit”。
- 用户询问提交流程、知识库覆盖检查、`--no-verify`、`git add -A` 等提交流程约束。

## 核心规则

1. **默认模式**：步骤 2 必须检查知识库覆盖，从 `git diff HEAD` 与 untracked 文件推断能力模块，并对照 `.flow-spec/topics/` 与 `.flow-spec/stock-docs/`。
...
- 技能全文：`skills/fs-git-commit/SKILL.md`
- 模板源：`templates/skills/fs-git-commit/SKILL.md`
- 命令说明：`docs/命令说明.md`
