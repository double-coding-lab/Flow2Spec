...
description: 据 req-docs、git log、.task 与知识库主题语义生成里程碑（《项目里程碑模版》）；触发：fs-doc-milestone、生成项目里程碑、里程碑。命令后可附语义化范围。本技能固定子 agent 生成、主 agent 验证，不受 flow-spec.config 编排开关影响
---

> 执行口径：读 `.flow-spec/template/项目里程碑模版.md`；落盘 **仅** `.flow-spec/stock-docs/<范围名>里程碑.md`（无第二路径参数）。

## 编排（固定，不受项目配置影响）

**本技能不受** `flow-spec.config.json` 中 **`subAgent`**、**`switchAgentVerification`**（及旧键 `subAgentVerification`）**影响**：无论其为 `true` 或 `false`，**一律**按下述分工执行，**禁止**因配置改为「全主会话」或「子 agent 自验即结束」。
...
> 步骤 0 仍 **`Read("flow-spec.config.json")`**（满足 `fs-config-check` 前置），但**不得**用其中的 `subAgent` / `switchAgentVerification` 改变本技能编排。
...
| `template` | `.flow-spec/template/项目里程碑模版.md`（不写模版顶部说明 blockquote） |
...
| 源 | 读什么 | 里程碑里怎么用 |
| --- | --- | --- |
| **req-docs** | 范围内 `.flow-spec/req-docs/*.md` | 需求/方案节点、交付摘要 |
...
1. **`Read(".flow-spec/manifest-routing.json")`**：提取 `topicPaths`、`taskToTopicRules`（及与范围相关的 `topicDependencies`）。
2. **`Read(".flow-spec/index.md")`**：至少「**主题一览**」表（主题 id、适用场景、关联文档摘要）。
3. **按需 `Read` `.flow-spec/topics/<topic>.md`**：与范围或 manifest 命中相关的摘要（**禁止**为枚举遍历整个 `topics/`；仅读 manifest/index 已点名的主题，通常 ≤ 全表行数）。
...
1. **`Read("flow-spec.config.json")`**（不采纳其 `subAgent` / `switchAgentVerification` 编排本技能）
2. **`Read(".flow-spec/template/项目里程碑模版.md")`**
3. **主题索源**（见上：manifest → index 主题一览 → 按需 topics 摘要）
4. 解析范围 → 确定默认路径 **`stock-docs/<范围名>里程碑.md`**。
5. **相似文件检查（落盘前必做）**：列出 `.flow-spec/stock-docs/` 下已有 `*里程碑*.md`（含 `*里程碑.md`）。若存在与本次**目标路径相同**或**语义相近**的文件（例如同为「整个项目」的 `项目里程碑.md` 与另一份全项目里程碑、或前缀/范围关键词高度重叠），**须先询问用户**，**禁止**静默覆盖或擅自另存：
