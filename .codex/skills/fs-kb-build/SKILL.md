---
name: fs-kb-build
description: 根据 .flow-spec/stock-docs 文档生成知识路由主题与索引；触发：生成项目上下文、fs-kb-build、终稿生成上下文
---

> 执行口径：本技能只维护 `.flow-spec`（`topics/index/manifest-routing/matchers` 分片），不改配置根 `rules/skills`。不再维护 `.flow-spec/manifest-matchers.json`（已废弃聚合文件；`flow-spec init` 会删除遗留副本）。

# 根据文档生成项目上下文（topics/index/路由清单）

## 编排（主 / 子 agent）

- 两字段（`subAgent` / `switchAgentVerification`）语义以统一入口为唯一事实源：**Cursor/Claude** 读配置根 `rules/fs-flow-spec-unified-entry.*`；**Codex** 读 `.codex/fs-rules/fs-flow-spec-unified-entry.md`（与上同源，`flow-spec init` 镜像）。本 SKILL 不复述。
- **首选分支（小变更 → 主全流程）**：当本次改动 **≤ 2 个新 / 改主题**，**且 ≤ 1 个新 matcher**，**且无跨主题批量引用调整** 时，全流程在主 agent 完成，不拆子。
- **中大变更分支**（`subAgent=true` 且超出上述阈值）：
  - 主 agent 在主会话中列出**文件级契约**：子 A 只写 `.flow-spec/topics/<foo>.md`，子 B 只写 `.flow-spec/matchers/<m-foo>.json`，路径互不重叠；
  - 子 agent 仅落盘契约内文件，不跨边界；
  - **主 agent 单点**编辑 `.flow-spec/manifest-routing.json` / `.flow-spec/index.md`（补 `taskToTopicRules`、`topicPaths`、`matcherPath`、`topicDependencies`、`topicMetadata`）；
  - 主 agent 做整体验收。
...
- **写权硬约束**：`.flow-spec/manifest-routing.json`（含 `topicMetadata`）/ `.flow-spec/index.md` **恒由主 agent 落盘**，子 agent 不得触碰。
- 默认落盘侧 agent 自验；本 SKILL 不绑定交叉校验。

## 输入

- 接收一个参数：URL 或本地路径。
- 本地路径必须位于 `.flow-spec/stock-docs/`。
- **须为终稿**：推荐文件名含 `_终稿.md`，或已由 **`fs-doc-final`** 规范化；**禁止**以 `fs-doc-arch` 产出的 `*_初稿.md` 作为入参直接执行本技能。
- 若入参路径含 **`_初稿`**、或用户刚完成架构初稿尚未执行 `fs-doc-final`：**停止**，回复须先执行 **`fs-doc-final <初稿路径>`**，待终稿落盘后再以终稿路径调用本技能。
- 若传入 `.flow-spec/req-docs/`，提示用户先整理为 `stock-docs` 终稿后再执行。
...
- URL：抓取正文；无法访问时提示用户先落地到 `.flow-spec/stock-docs/*.md`。
...
> **创作侧准则**：本步骤涉及新增 / 修改 topic 与 `topicDependencies`，**须先 Read** `rules/fs-topic-authoring.*` 全文（**Cursor/Claude**：`rules/fs-topic-authoring.mdc`；**Codex**：`.codex/fs-rules/fs-topic-authoring.md`），再继续步骤 3 / 步骤 5。命名、骨架、依赖判定、DAG 最小化、判定时机均以该条为准，本 SKILL 不复述。
...
- 目标路径：`.flow-spec/topics/<topic>.md`
- 若已存在同主题：优先增量更新，避免重复主题。
- 若为新主题：新增文件并补充清晰标题、适用场景、规则与流程。

## 步骤 4：更新 index

- 更新 `.flow-spec/index.md` 的主题路由表。
- 保证“同主题单行”。
...
- `sourceDoc` 或文档引用统一指向 `.flow-spec/stock-docs/<文件名>.md`
- 禁止把 `.flow-spec/req-docs/` 作为 topic 的 `sourceDoc`
...
用户输入：`fs-kb-build .flow-spec/stock-docs/<能力>_终稿.md`，且现有 `topics/<能力>.md` 已存在。
...
1. `.flow-spec/topics/*.md` 与 `manifest-routing.topicPaths` 一一对应。
...
7. `manifest-routing.json` / `.flow-spec/index.md` 由主 agent 单点落盘，无子 agent 越权写入。
