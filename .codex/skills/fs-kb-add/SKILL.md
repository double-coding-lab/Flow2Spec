---
name: fs-kb-add
description: 工作中把已落地能力解析进知识库（多文件聚合）：初稿→终稿→topics/index/manifest；触发：fs-kb-add、已有能力进知识库、多文件生成上下文
---

> 执行口径：本技能只维护 `.flow-spec`，不改配置根 `rules/skills`。

## 编排（主 / 子 agent）

- `subAgent` / `switchAgentVerification` 两字段语义以统一入口为唯一事实源：**Cursor/Claude** 读配置根 `rules/fs-flow-spec-unified-entry.*`；**Codex** 读 `.codex/fs-rules/fs-flow-spec-unified-entry.md`（与上同源，`flow-spec init` 镜像）。
...
- **拆子策略（仅在达到拆子阈值且 `subAgent=true` 时启用）**：
  - **B 模式（默认，单轮并行）**：主先产出「inventory（待解析源文档路径清单 + 核心能力名，主手写，禁止子 agent 自行增删）」+「扫描契约（每个源读哪些章节 / 行号范围、禁扫目录、统一产出字段与表头）」→ 子 agent 并行只读按表填写 → 主一轮合并 + 去重 → 写 `.flow-spec/stock-docs/<方案名>_初稿.md` → 主做用户确认与验收。适合源边界较清晰、中等规模、希望尽快出一版。
...
- **子交付硬约束**：子 agent 不得自行裁剪源路径范围，必须按主手写 inventory 执行；交付按「子交付 YAML schema」（字段：`source` / `scope` / `capabilities` / `cross_refs` / `pending`），禁止散文式回传；子不得写 `manifest-routing.json` / `.flow-spec/index.md`；子不得单独宣布「已进知识库」。
- 主必控：重合判定、终稿定稿、`fs-kb-build` 调度、整体验收。
- 写权硬约束：`manifest-routing.json` 与 `.flow-spec/index.md` 恒由主 agent 落盘。
- 落盘侧自验。

# fs-kb-add：多文件聚合 -> 初稿 -> 终稿 -> 知识路由同步

## 使用时机

- 某能力已在代码中落地，但信息分散在多个文件，需沉淀为可检索知识。
- 与 `fs-doc-arch` 区分：`doc-arch` 产出架构初稿；`doc-add` 产出“已落地能力”知识沉淀链路。
...
| 初稿/终稿路径 | 否 | 默认放 `.flow-spec/stock-docs/` |
...
- 默认输出：`.flow-spec/stock-docs/<方案名>_初稿.md`
- 初稿建议结构：
  - 概述
  - 来源清单（含不可读文件）
  - 分模块归纳
...
- 参考 `.flow-spec/template/终稿模版.md`
- 输出：`.flow-spec/stock-docs/<方案名>_终稿.md`
- **必须填写 `## 来源文件` 小节**，列出步骤 1 实际读取的原始源文件路径
- 若用户要求”先审初稿”，则停在初稿并等待确认

## 步骤 4：同步知识路由

基于终稿调用 `fs-kb-build` 口径，更新：
...
> **创作侧准则**：本步骤会触发新增 / 修改 topic 与 `topicDependencies`，**须先 Read** `rules/fs-topic-authoring.*` 全文（**Cursor/Claude**：`rules/fs-topic-authoring.mdc`；**Codex**：`.codex/fs-rules/fs-topic-authoring.md`），再调用 `fs-kb-build` 口径同步。
...
- 若发现已有 `.flow-spec/stock-docs/<能力名>_终稿.md`：优先在该终稿上修订，而不是新建重复终稿。
- 用户要求”先审初稿”：必须停在初稿，等待确认后再生成终稿并进入 `fs-kb-build` 同步。
...
- 终稿 `sourceDoc` 仅指向 `.flow-spec/stock-docs/*`
- 不改配置根 `rules/skills`
- 同主题优先更新，不平行新建重复知识
- `manifest-routing.json` 与 `.flow-spec/index.md` 恒由主 agent 落盘（写权硬约束），子 agent 不得触碰

## 完成后自检

1. 初稿/终稿路径是否落在 `.flow-spec/stock-docs/`。
2. 同主题是否避免重复新建。
