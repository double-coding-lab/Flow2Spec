...
<p align="center">
  ...
  <a href="https://double-coding-lab.github.io/Flow2Spec/presentations/flow2spec-intro-public-en/">Product deck</a> ·
...
After `/f2s-doc-arch`, if you already know **which modules the upcoming request will touch**, take one more step and pre-ingest those modules' existing notes, for example:

```text
/f2s-kb-add src/services/product-review src/functions/batch-rescore
```

That way, before you even start the real change, `.Knowledge` already contains: the whole-repo view (from `f2s-doc-arch`) plus the internal constraints of the modules you're about to touch (topic + matcher generated by `f2s-kb-add`). What the agent reads in later turns is focused project facts, not scattered source code.
