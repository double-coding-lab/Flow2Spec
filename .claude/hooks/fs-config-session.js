...
'use strict';
/**
 * flow-spec SessionStart hook — 会话开始时一次性注入 flow-spec.config.json 摘要。
 * 该摘要不替代 fs-* Skill 正文前的 Read("flow-spec.config.json")。
 * 由 flow-spec init 写入对应 agent 的 hooks/fs-config-session.js。
 */
const fs = require('fs');
...
function main() {
  ...
  if (!fs.existsSync(configPath)) {
    ...
    emit([
      '[flow-spec] 本会话未找到 flow-spec.config.json；fs-* Skill 前仍必须尝试 Read("flow-spec.config.json")，缺失字段按默认值处理。',
  ...
  try {
    ...
    emit([
      '[flow-spec] SessionStart 配置摘要（仅作提醒，执行 fs-* Skill 前仍必须 Read 磁盘文件）：',
    ...
    emit([
      `[flow-spec] flow-spec.config.json 解析失败：${err.message || String(err)}`,
      '执行 fs-* Skill 前必须先修复或 Read 该文件；无法读取时按缺失字段默认值处理。',
    ]);
  }
}
