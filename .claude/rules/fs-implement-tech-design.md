---
description: 当用户要求根据技术方案文档实现可运行交付物时，按本规则执行（读文档、列任务、确认、实现、待完成列表与提醒）。用户会在对话中提供技术方案文档路径（MD 或 PDF）；若为 PDF，先按 fs-doc-pdf 转为 MD 再继续。
paths:
  - "**/.flow-spec/req-docs/**/*.md"
---

> **唯一长文**：本文件为 **implement-tech-design** 的完整执行条令。`.flow-spec/topics/fs-implement-tech-design.md` 仅为路由摘要；**Codex** 读取 `.codex/fs-rules/fs-implement-tech-design.md`（由 `flow-spec init` 从本文件自动镜像）作为等效条令。

> 执行口径：统一知识库路径为 `/.flow-spec/`。下文所有路径均按 `.flow-spec` 约定解释。

# 基于技术方案实现交付物（通用）

当用户要求根据**技术方案文档**实现可运行交付物时（用户会提供文档路径，如 `.flow-spec/req-docs/xxx.md` 或 PDF），按以下约定执行。

**目录约定**：`.flow-spec/req-docs/` 放“用于实现”的技术方案；`.flow-spec/stock-docs/` 放沉淀文档，不作为直接编码输入。

**触发说明**：本规则在打开 `req-docs` 下 `.md` 时自动加载（`**/req-docs/**/*.md`）。若对话前未打开技术方案，可在对话中 @ 本规则后再提供路径。

- 若用户提供的是 PDF：先执行 `fs-doc-pdf`，将 PDF 转为 `.flow-spec/req-docs/` 下 MD，再继续。
...
- PDF 输入：先执行 `fs-doc-pdf`，得到 `.flow-spec/req-docs/*.md`。
- MD/文本输入：直接读取。

### 步骤 2：理解方案与上下文

1. 读取技术方案全文，提取：目标、范围、流程、接口/交互、数据、配置、依赖、验收条件。
2. 读取项目约定（如 README、`.flow-spec/stock-docs/`、架构说明、既有模块）以对齐实现风格。
...
若 `changeTracking.implement: true`，在输出任务列表后，按 `fs-task` 规则将本清单写入 `.task/active/<task-name>/task.md`。

### 步骤 2.6：变更追踪与 `task.md` / `user-todos.md` 同步（仅当 `changeTracking.implement: true`）

- 每完成实现任务列表中一项对应工作，**同一会话内**用 `Edit` 更新 `.task/active/<task-name>/task.md` 中对应 `[ ]`→`[x]`，禁止积压到收尾、禁止口头完成代替写盘（见 `fs-task`「执行中」「中断与会话结束」）。
- 执行过程中每出现**须用户执行**的项（改库、配环境等），**同会话内**追加到 `.task/active/<task-name>/user-todos.md`（见 `fs-task`「user-todos.md」）。
...
4. **用户代办落盘（仅当 `changeTracking.implement: true`）**：将步骤 5 第 1–2 点中**须用户亲自执行**的条目（改库脚本、配置、审批等）**同步追加**到 `.task/active/<task-name>/user-todos.md`（若尚无该文件则先创建，见 `fs-task`）；禁止仅出现在对话或方案尾部的列表而不写入该文件。
5. 若 `changeTracking.implement: true`：**先确认** `task.md`「步骤」已全部 `[x]`（或备注已记录取消项），满足 `fs-task` 归档门禁后，再将 `.task/active/<task-name>/` 移至 `.task/completed/<YYYYMMDD>-<task-name>/`，并从 `todo.json` 删除对应条目；禁止在仍有 `[ ]` 时归档。
...
- 若用户希望沉淀知识库，可提醒后续用 `fs-kb-build` 同步主题与路由。
...
- 若 `changeTracking.implement: true`：不得跳过步骤 2.6（随实现进度写回 `task.md` checkbox，并追加 `user-todos.md`）；归档须满足 `fs-task` 归档门禁；
