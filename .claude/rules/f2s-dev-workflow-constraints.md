---
description: Flow2Spec 项目开发纪律（仅本双仓适用）：修改本仓时只改 templates/，配置根由 flow2spec init / sync:agents 分发，禁止手改配置根；双仓改动须同步
---
...
- **B 类：只有 Flow2Spec 双仓自己需要的开发纪律**（比如「不要手改配置根」「改完记得双仓同步」「用户来跑 init」）——**只放本仓配置根 + 本仓 `.Knowledge/topics/`**，`init` **不带**到下游。
...
本仓配置根手写内容（本文件、f2s-dev-workflow-check skill）
...
- **本仓专属规则 / 技能**：`.claude/rules/f2s-dev-workflow-constraints.md`、`.cursor/rules/f2s-dev-workflow-constraints.mdc`、`.codex/topics/f2s-dev-workflow-constraints.md`、`.claude/skills/f2s-dev-workflow-check/`、`.cursor/skills/f2s-dev-workflow-check/`、`.codex/skills/f2s-dev-workflow-check/`——这些**从不进 `templates/`**，直接手写落三端配置根
...
**注意**：`init` **不会**动本仓专属手写文件（本规则、`f2s-dev-workflow-check` skill）——这些文件在 `templates/` 里没有对应源，`init` 找不到就不写，用户手写的原始版本得以保留。
...
- **`f2s-git-commit`**（依赖主题）：本规则第 5 条「提交前须确认双仓一致」在 `f2s-git-commit` 的提交流程里落地。
...
- 禁止把本纪律当作「使用 flow2spec 的下游项目」的通用约束——**本文件仅针对 Flow2Spec 双仓自身的开发**。
