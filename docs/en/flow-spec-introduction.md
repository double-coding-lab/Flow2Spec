[中文](../flow-spec基础介绍.md) | [English](./flow-spec-introduction.md) · [项目首页](../../README.zh-CN.md) | [Project home](../../README.md)

# flow-spec: Let Projects Naturally Grow a Knowledge Graph During Development

<p><img src="../images/flow-spec-intro-cover.png" alt="flow-spec cover" style="max-width:720px;width:100%;" /></p>

---

## I. Introduction

<p><img src="../images/flow-spec-intro-01-preface.png" alt="Introduction" style="max-width:720px;width:100%;" /></p>
...
That's flow-spec. In one line: **flow-spec is an Agent engineering framework that lets a project naturally grow a knowledge graph during development.**

---

## II. Why "Memory" Alone Isn't Enough

<p><img src="../images/flow-spec-intro-02-memory.png" alt="Why memory alone isn't enough" style="max-width:720px;width:100%;" /></p>
...
flow-spec's position is: **project knowledge can't just be "written down" — it needs to be routable, composable, verifiable, and continuously updated.**

---

## III. flow-spec's Key Difference: A Knowledge Graph That Grows During Development

<p><img src="../images/flow-spec-intro-03-knowledge-graph.png" alt="Knowledge graph growing during development" style="max-width:720px;width:100%;" /></p>

flow-spec doesn't ask you to do a massive documentation effort upfront. The recommended approach is:

1. Run `flow-spec init` to initialize an empty skeleton.

2. Use `fs-doc-arch` to generate an architecture overview and bring it into the knowledge base.

3. When real requirements arrive, have the Agent route through existing knowledge first.

4. After a capability ships, use `fs-kb-sync` to sync confirmed facts back into the knowledge base.
...
This is what sets flow-spec apart from ordinary "project memory files": an ordinary solution hands the Agent a manual; flow-spec maintains an evolvable knowledge graph in the repository's `.flow-spec/` — diffable, reviewable, committed alongside code.

---

## IV. The Knowledge Base Interface: Not a Pile of Docs, but a Routing Protocol

<p><img src="../images/flow-spec-intro-04-routing-protocol.png" alt="Knowledge base routing protocol" style="max-width:720px;width:100%;" /></p>

flow-spec's knowledge base has a clear interface structure:
...
<p><img src="../images/flow-spec-intro-05-progressive-reading.png" alt="Incremental retrieval" style="max-width:720px;width:100%;" /></p>

flow-spec's retrieval model can be summarized in four steps: **match → expand → verify → act**
...
<p><img src="../images/flow-spec-intro-06-dependencies.png" alt="Multi-dependency capability" style="max-width:720px;width:100%;" /></p>
...
flow-spec makes these dependencies explicit in the routing layer. A topic can declare which other topics it depends on; when the Agent hits a primary topic, it expands dependencies first. This isn't just "read a few more files" — it transforms project knowledge from flat documents into a graph with edges:
...
<p><img src="../images/flow-spec-intro-07-knowledge-validation.png" alt="Knowledge base correctness" style="max-width:720px;width:100%;" /></p>

Two things kill a knowledge base: going stale, and being wrong. flow-spec doesn't assume the knowledge base is always correct — it builds verification into the development process.

A typical scenario: the user asks a business detail; the Agent checks the knowledge base, finds topic coverage but not enough detail, then drills into source code for a more accurate fact. At this point flow-spec shouldn't just answer and move on — it also needs to determine:

- Has this fact already been written into the topic?

- If not (or coverage isn't detailed enough), should it suggest `fs-kb-distill` to extract this round's Q&A into the knowledge base?
...
<p><img src="../images/flow-spec-intro-08-intent-recognition.png" alt="User intent recognition" style="max-width:720px;width:100%;" /></p>
...
flow-spec has an `intentRecognition` switch. When enabled, the Agent uses intent recognition rules to assist routing: high-confidence new capability → feat workflow; high-confidence bug fix → fix workflow; unclear requirements → req\-clarify; mere asking or discussing → stay in normal conversation.

After validation across real projects over time, **intent recognition is now stable enough that we recommend enabling it by default**, letting the Agent route automatically in most scenarios; you can still type `fs-req-clarify`, `fs-kb-feat`, `fs-kb-fix`, etc. explicitly at any time to override the automatic decision. We also recommend using `fs-*` skills explicitly more often — getting familiar with them is the most efficient way to use flow-spec.

---

## IX. A More Realistic Development Loop

<p><img src="../images/flow-spec-intro-09-dev-loop.png" alt="Development loop" style="max-width:720px;width:100%;" /></p>

With flow-spec, a requirement might flow like this:

```Plain Text
User submits requirement
  -> Explicit fs-* skill / intentRecognition assists
  -> fs-req-clarify clarifies until no ambiguity
  -> fs-req-tech generates technical design
  -> Agent reads knowledge base incrementally
  -> Implements code
  -> changeTracking records task progress
  -> fs-kb-sync syncs new knowledge
  -> fs-git-commit pre-commit check
...
```mermaid
flowchart TD
  A[User submits requirement] --> B{Explicit fs-* skill<br/>or intentRecognition}
  B -->|unclear requirement| C[fs-req-clarify<br/>Requirements clarification]
  B -->|new capability| D[fs-kb-feat<br/>Add capability]
  B -->|bug fix| E[fs-kb-fix<br/>Fix issue]
  B -->|large requirement / needs design| F[fs-req-tech<br/>Technical design]
  C --> F
  ...
  H --> J[fs-kb-sync<br/>Sync confirmed facts]
  J --> K[.flow-spec<br/>Knowledge graph incremental update]
  I --> L[fs-git-commit<br/>Pre-commit check]
  K --> L
...
**flow-spec isn't about making the AI answer better in a single session — it's about turning each development process into an incremental update to the project's knowledge graph.**

---

## X. It Manages More Than Knowledge — It Manages the Development Loop

<p><img src="../images/flow-spec-intro-10-dev-closure.png" alt="Development loop management" style="max-width:720px;width:100%;" /></p>
...
**Technical designs don't follow a rigid template**: `fs-req-tech` selects structure based on the current requirement, rather than mechanically filling every section — so a frontend change doesn't generate a template full of database chapters.
...
**Pre-commit knowledge coverage check**: `fs-git-commit` checks the diff, conflict markers, and staging scope before committing, and checks whether the round's changes require a knowledge base sync — catching "changed code, forgot to update knowledge" before the commit lands.

**Template and routing upgrade detection**: At startup, flow-spec checks whether `.flow-spec/` is behind the npm package version and, if so, prompts `fs-kb-upgrade` to keep the project's knowledge structure aligned with the tool version.

---

## XI. The Biggest Difference from Ordinary Knowledge Bases

<p><img src="../images/flow-spec-intro-11-vs-ordinary-kb.png" alt="vs ordinary knowledge bases" style="max-width:720px;width:100%;" /></p>

In one sentence: **an ordinary knowledge base is something Agents query; flow-spec's knowledge base is something Agents help maintain.**

Figure 3: Ordinary Memory Files vs flow-spec Knowledge Graph
...
flow-spec focuses more on: which topic to read when a requirement arrives, what dependencies exist between topics, whether current knowledge is sufficient to act, whether to feed knowledge back after a source code dive, whether user intent should trigger a workflow, whether knowledge gets updated alongside code changes, and whether knowledge coverage was checked before committing.

This is why flow-spec includes `.flow-spec/`, `.task/`, `fs-*` skills, Agent rules, and `flow-spec.config.json` together — they aren't separate pieces, but a protocol organized around the development loop.

---

## XII. Common Questions

<p><img src="../images/flow-spec-intro-12-faq.jpg" alt="Common questions" style="max-width:720px;width:100%;" /></p>

**Q: After changing a capability, how do I make sure all related topics get updated?**

flow-spec doesn't promise "the model will automatically know all impact areas" — it turns impact discovery into an executable process.
...
> flow-spec's goal isn't to have the Agent intuitively edit one file — it's to first list which topics this change might affect, confirm, then write to disk.

---

**Q: How does flow-spec solve the problem of Agents forgetting rules?**
...
Two typical scenarios show how these layers work: preventing intent misfire (user still clarifying — Agent must not jump to implementation); and preventing knowledge from not being fed back after a source code answer (must determine whether `fs-kb-distill` is needed to extract this round's Q&A into the knowledge base — silent skipping not allowed).

flow-spec's goal isn't to completely eliminate forgetting — it's to make "bypassing rules" harder at every stage.
...
flow-spec offers three complementary commands, distinguished by who triggers them and at what granularity:

`fs-kb-distill` is **auto-suggested by the Agent**: after a single Q&A drills into source code, it extracts this round's Q&A into the KB; internally the skill decides whether to create a new topic or supplement an existing one based on drill-down depth and topic hits.

`fs-kb-add <path or capability>` is **user-initiated**: use it when you want to parse an **entire module / legacy capability** into the KB in one go — accepts a path or multi-file aggregation, ideal for never-indexed blocks of code.

`fs-kb-sync` is **user-initiated**: used for **global / batch sync** of shipped capabilities; supports zero-input inference and outputs an update outline before writing — ideal for periodic checkup-style reinforcement.
...
<p><img src="../images/flow-spec-intro-13-quick-start.png" alt="What projects and quick start" style="max-width:720px;width:100%;" /></p>
...
When getting started, we recommend typing `fs-*` skills explicitly first to get familiar with them — it's the most efficient way to learn flow-spec. Enabling `intentRecognition` by default then lets the Agent route common tasks automatically.

Common workflows:

```Plain Text
/fs-req-clarify   Requirements clarification
/fs-req-tech      Generate technical design
/fs-kb-feat       Add capability and sync knowledge
/fs-kb-fix        Fix issue and correct knowledge
/fs-kb-add <path> Parse an existing module into the knowledge base
/fs-kb-sync       Global / batch sync of shipped capabilities into the KB
/fs-kb-distill    Extract this round's Q&A into the KB (auto-decides new topic or supplement)
/fs-git-commit    Pre-commit check and generate commit message
...
<p><img src="../images/flow-spec-intro-14-conclusion.png" alt="Closing" style="max-width:720px;width:100%;" /></p>
...
flow-spec turns project knowledge from static documents into a routable, dependency-aware, verifiable, feedback-capable knowledge graph. It makes Agents not just consumers of context, but participants in maintaining it.
...
Repository: `https://github.com/Lands-1203/flow-spec`

Live demo: `https://lands-1203.github.io/flow-spec/`

---

<p><img src="../images/flow-spec-intro-15-star.png" alt="GitHub Star" style="max-width:720px;width:100%;" /></p>

If flow-spec has been helpful, consider giving it a Star on GitHub — your support keeps the project going!
