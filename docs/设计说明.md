...
| 普通问答源码补答收口                        | `f2s-kb-feedback-closing`     | 源码补答后强制四 case 显式表态：case 1～3 输出 `f2s-kb-distill` 补充建议，case 4 输出"知识库已覆盖"显式标记；禁止悄悄跳过整个收口流程。 |
...
graph LR
    AI["下次会话\n的 AI"] --> C["功能迭代"]
    K[".Knowledge/"] 
    K1[".Knowledge/"] --> AI["下次会话\n的 AI"]

    C -->|"修复 Bug"| FIX["f2s-kb-fix"] --> K1
    C -->|"新增能力"| FEAT["f2s-kb-feat"] --> K1
    C -->|"会话结束"| SYNC["f2s-kb-sync"] --> K1
...
`.task/` 早期跟普通项目文件一样进入 Git。单人使用时问题不大；多人开始共用仓库后，`todo.json` 和 `active/` 暴露了它们的真实属性：这是某个开发者、某次 Agent 会话的工作现场，不是项目事实。

于是 `.task/` 的所有权从“仓库共有”改成“开发者本地持有”：

```text
旧模型
  .task/ -> Git -> 所有人共享同一份 todo.json

现模型
  .task/alice/ -> Alice 本地续作
  .task/bob/   -> Bob 本地续作
  .Knowledge/  -> Git -> 团队共享事实
```

这次变化解决的不是文件冲突本身，而是“谁有资格声明这份状态”的问题。任务做到第几步，由执行者自己的 `TASK_ROOT` 负责；能力最终是什么，由代码、正式文档和 `.Knowledge/` 负责。前者可以随会话变化，后者必须经过团队评审。

`developerId` 仍保留在本地路径中，用来覆盖共享开发机、devcontainer 和 Agent 越界读取等场景。它不是远程账号体系，也不承担团队权限管理。

### 6. 知识合并为什么用乐观锁

`.Knowledge/` 必须共享，不能像 `.task/` 一样按人拆开。这里采用 topic revision 乐观锁，而没有引入文件锁或远程锁服务。

文件锁要求所有写入者在修改前都能看到同一个在线锁状态，还要处理租约、断线、锁遗留和 CI 身份。Flow2Spec 的协作单元却是 Git 分支：开发者可以离线工作，Agent 也可能在不同 worktree 中运行。为了一个 Markdown 知识库再部署锁服务，会让本地优先的工作流反过来依赖中心节点。

乐观锁利用了仓库已经拥有的磁盘真值。delta 记录“我读取时 topic revision 是多少”，`plan` 在写入前比较当前 revision：

```text
baseRevision == diskRevision  -> 可以 apply，revision +1
baseRevision != diskRevision  -> 停止，重读最新 topic，做语义合并
```

代价也很明确：锁是 topic 级，两个互不相关的段落只要落在同一 topic，仍会互相阻塞；本地没拉到远端最新提交时，`plan` 也不可能感知远端变化。Flow2Spec 接受这两个限制，换取无服务、可离线、可审阅的实现。频繁冲突的正确处理通常是缩小 topic 职责，而不是把锁细化到行级后掩盖业务语义冲突。
...
```mermaid
graph TD
    A["flow2spec init codex"] -->|写入| B["hooks/f2s-update-check.js"]
    A -->|写入| C["hooks.json"]
...
| **Codex `AGENTS.md` / `.codex/topics/f2s-config-check.md` + `f2s-config-session`** | `SessionStart` 注入一次配置摘要，文本层强制「技能正文前先 Read」；仍无 Claude 式 `PreToolUse Skill` 守门。     |
...
**权威仍为**项目根 JSON 的 **Read** 结果；各层为提示而非第二份真值源。操作侧完整表格与路径见 **[使用说明 § 一、`f2s-*` 与 `flow2spec.config.json`](./使用说明.md)**；口述节奏见 **[Flow2Spec 演讲稿 Slide 13b](../presentations/flow2spec-intro-public/演讲稿.md)**。
...
- [Flow2Spec 演讲稿](../presentations/flow2spec-intro-public/演讲稿.md)
- [项目里程碑](./项目里程碑.md)
