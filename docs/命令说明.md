...
**工作原理**：通过「版本分流 + init 代跑」实现升级——先检测当前知识库属于 V1（旧结构）还是 V2+（已有 `.Knowledge`）：V1 已不再内置迁移支持，技能停止并告知用户处理方式（用历史版本包一次性迁移，或手动迁入 `.Knowledge`）；V2+ 直接代跑 `flow2spec init` 进行包级结构增量对齐（补齐新模板、升级 manifest schema、对齐 matchers 分片格式）。升级后重读 SKILL.md 判断是否需要重跑某些步骤。与单独 `init` 的区别：`kb-upgrade` 包含版本判断和重跑逻辑，`init` 仅做一次性结构补齐；**非主题版本更新**（init 后 `projectRev == pkgRev`）可由 agent 直接代跑 `flow2spec init` 完成，无需进入本技能完整流程。
