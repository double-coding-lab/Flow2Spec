...
| 命令 | 用途 | 分类 |
|---|---|---|
| `/fs-doc-arch` | 扫描项目，生成架构说明初稿 | 文档沉淀 |
| `/fs-doc-final` | PDF / 初稿转《终稿模版》规范格式 | 文档沉淀 |
| `/fs-kb-build` | 终稿文档同步到知识库路由（生成 topics / matchers / manifest） | 文档沉淀 |
| `/fs-kb-add <路径>` | 已落地能力多文件聚合入知识库 | 文档沉淀 |
| `/fs-kb-rm` | 删除某 stock-docs 文档对应的知识主题与索引映射 | 文档沉淀 |
| `/fs-doc-pdf` | PDF 技术方案转 Markdown，保存到 req-docs | 文档沉淀 |
| `/fs-req-clarify` | 需求澄清，多轮问答明确需求边界 | 需求与方案 |
| `/fs-req-tech` | 基于澄清结果生成技术方案文档 | 需求与方案 |
| `/fs-req-plan` | 按技术方案拆任务清单，支持并行实现 （不依赖 `changeTracking.*` 配置） | 需求与方案 |
| `/fs-git-commit` | 提交代码，默认检查知识库覆盖；说“快捷提交”时跳过覆盖检查 | 提交 |
| `/fs-kb-feat` | 新增能力，补全实现 + 同步知识库 | 知识库维护 |
| `/fs-kb-fix` | 修复 BUG + 自动同步知识库 | 知识库维护 |
| `/fs-kb-distill` | 从问答过程中提取可复用知识事实并自动入库 | 知识库维护 |
| `/fs-kb-sync` | 将会话中已实现能力沉淀回知识库 | 知识库维护 |
| `/fs-kb-merge` | 解决 Git 合并后的知识库冲突 | 知识库维护 |
| `/fs-kb-migrate` | 旧版知识库一次性迁移到 `.flow-spec/` | 知识库维护 |
| `/fs-kb-upgrade` | 知识库模板升级，对齐 manifest + matchers 分片 | 知识库维护 |
...
- **前置**：无
- **后续**：`fs-doc-final`（规范化终稿）→ `fs-kb-build`（**须终稿入参**；初稿不可直驱 build）
- **输出**：`.flow-spec/stock-docs/<架构说明>_初稿.md`
...
**工作原理**：将非结构化或格式各异的文档（PDF/初稿）对照内置终稿模版进行格式归一化：提取核心概念表、业务规则、关键流程、配置与错误处理等标准章节，补齐缺失段落标记，最终输出格式统一的 `_终稿.md`。终稿是 `fs-kb-build` 的标准输入物，确保知识库入口的结构一致性。

**使用场景**：

- PDF 技术方案需要转为 Markdown
- 初稿需要规范化以便沉淀
- 外部文档需要纳入 flow-spec 管理

**关联关系**：

- **前置**：PDF 文档或初稿文档
- **后续**：`fs-kb-build`（终稿入库）
- **输出**：`.flow-spec/stock-docs/<文档>_终稿.md`
...
- **前置**：`fs-doc-arch`、`fs-doc-final` 或直接编写的终稿
- **后续**：无（入库完成后可直接使用）
- **输入**：`.flow-spec/stock-docs/*.md`
- **输出**：
...
**工作原理**：从多个分散的源文件（代码、配置、散落文档）中聚合提取能力描述，走完整的「初稿→终稿→topics/index/manifest」沉淀链路。与 `fs-kb-build` 的区别在于输入：`ctx-build` 从已有的单份终稿驱动，`doc-add` 从多个散落源聚合后再走同一管线。本质是补齐「有实现无文档」的缺口。
...
**工作原理**：`fs-kb-build` 的逆操作——给定一份 `stock-docs` 文档路径，定位其在 `manifest-routing.json` 中的 task→topic 规则、对应的 `matchers/<id>.json` 分片、`topics/<topic>.md` 文件以及 `index.md` 中的行项，逐一清除引用。若删除后某 topic 无任何 task 引用，则移除该 topic 文件。源文档本身保留不动，用户可自行决定是否物理删除。
...
**工作原理**：将 PDF 中的接口定义、数据模型、时序流程等提取为 Markdown 并落盘 `req-docs/`，便于后续编辑或与 `fs-req-clarify` / `fs-req-tech` 衔接。与 `fs-doc-final` 的区别：`doc-pdf` 输出到 `req-docs/`（实现侧文档目录），`doc-final` 输出到 `stock-docs/` 供 `ctx-build` 入库。**不推荐**把「PDF 转 MD」当作跳过澄清与技术方案的直驱编码路径。

**使用场景**：

- 跨团队交付物为 PDF，需转为可编辑的 Markdown
- 历史 PDF 技术方案需纳入 `req-docs/` 管理
- 为后续 `fs-req-clarify` / `fs-req-tech` 提供可读底稿

**关联关系**：

- **前置**：PDF 文档
- **输出**：`.flow-spec/req-docs/<方案>.md`
- **下一步**（推荐）：`fs-req-clarify` → `fs-req-tech` → 按 `req-docs/` 中技术方案 MD 触发 `implement-tech-design`；若目标是知识库沉淀则 `fs-doc-final` → `fs-kb-build`
...
**工作原理**：采用「结构化追问」策略——将需求文档按「角色/场景/流程/边界/异常/非目标」六维拆解，逐维检查是否存在模糊表述、未定义概念或矛盾点，对每个缺口生成针对性追问。多轮对话直到所有维度无歧义后，输出需求澄清记录作为 `fs-req-tech` 的输入。本质是将非结构化 PRD 转为可落地的结构化需求约束。
...
- **前置**：无（可直接触发）
- **后续**：`fs-req-tech`（澄清后生成技术方案）
- **输出**：需求澄清记录（可选保存至 `.flow-spec/req-docs/`）
...
- `fs-req-clarify` 完成后，基于澄清结果输出方案
- 已有明确需求文档，直接生成技术方案

**关联关系**：

- **前置**：`fs-req-clarify`（推荐）或明确的需求文档
- **输出**：`.flow-spec/req-docs/<技术方案>.md`
...
> **关于任务清单**：任务清单不是 `/fs-req-plan` 专属。`/fs-kb-feat`、`/fs-kb-fix` 和 `implement-tech-design` 在 `flow-spec.config.json` 中对应 `changeTracking.*` 字段为 `true` 时同样会自动创建任务清单。`/fs-req-plan` 的区别是**不依赖任何配置、始终创建清单**，适合用户明确希望追踪进度的大型需求。
...
- **前置**：技术方案文档路径（`.flow-spec/req-docs/*.md` 或 PDF）或需求/变更描述
- **输出**：`.task/active/<task-name>/task.md` + `context.md`；实现代码
- **后续**：可按需调用 `fs-kb-sync` 补充知识库
...
| 子 agent（实现） | 按模块实现代码，不触碰 `.task/` 和 `.flow-spec/` |
...
**工作原理**：在 `git commit` 之上叠加「知识库覆盖门控」——先通过 `git diff` 推断本次变更涉及的功能模块，再与 `.flow-spec/topics/` 和 `stock-docs/` 交叉比对，判断变更能力是否已有知识库记录。未覆盖时阻断并提示三选（补录/跳过/取消），避免「代码有了但知识库不知道」的静默漂移。快捷提交模式只跳过这层覆盖门控，不跳过冲突检查、提交信息展示、精确 `git add`、git hooks。提交信息强制 emoji + Conventional Commits 格式，保证 git log 的机读一致性。
...
- **前置**：代码已写完（`implement-tech-design`、`fs-kb-fix`、`fs-kb-feat` 等执行后）
- **后续**：无（commit 完成即结束，不自动 push）
- **可衔接**：若知识库未覆盖，可先运行 `fs-kb-sync` 或 `fs-kb-feat` 补录后继续提交

**执行流程**：

1. `git status --short` + `git diff HEAD` 区分 staged / unstaged / untracked 三类文件；发现 merge conflict 标记立即终止
2. 默认对比 `.flow-spec/topics/` 与 `stock-docs/`，判断本次变更能力是否已入库；`.flow-spec` 不存在时跳过并提示；用户说“快捷提交”时跳过本步
...
**变更追踪**：若 `changeTracking.fix: true`，执行前自动检查 `.task/todo.json` 并创建任务清单，完成后自动归档；跨会话可通过关键词续作（见 `fs-task` 规则）。
...
**工作原理**：执行「判断→实现→入库」三阶段——先判断用户描述的能力在代码中处于「未实现/部分实现/已实现」哪种状态；未实现或部分实现时先补齐代码；最后走知识库同步：写 `stock-docs` 能力说明、生成或更新 `topics` 主题摘要、在 `manifest-routing` 和 `matchers` 注册路由映射。与 `fs-kb-fix` 的区别：`kb-feat` 面向「新增」，`kb-fix` 面向「修正已有」。
...
**变更追踪**：若 `changeTracking.feat: true`，执行前自动检查 `.task/todo.json` 并创建任务清单，完成后自动归档；跨会话可通过关键词续作（见 `fs-task` 规则）。
...
**工作原理**：在 Agent 通过源码回答用户问题后，自动分析本轮对话中出现的可复用知识事实（核心机制、状态流转、返回值契约、配置开关影响、失败回退策略、模块边界等），判断是否已被知识库覆盖：未覆盖则新建或扩充 topic；已覆盖但不够细则补充细节。与 `fs-kb-sync` 的区别：`sync` 适合批量同步多个能力；`distill` 专注于单次问答驱动的增量知识提取。
...
| 维度 | 条件 |
| --- | --- |
| 上游 `fs-kb-feedback-closing` case | **case 2 或 case 3**（case 1 / 无收口块 → 严格挡） |
| 本轮 Read 业务源码文件数 | **≤ 3 个** |
| 本轮回答引用函数 / 类名数 | **≤ 5 个** |
| 用户追问是否否定上游结论 | **否** |

**业务源码定义**：路径**不在** `.claude/` / `.cursor/` / `.codex/` / `.flow-spec/` / `.task/` 这 5 个目录下的 Read 才计数。
...
- 由 `fs-kb-feedback-closing` 规则自动建议调用
- 用户主动将本次问答结论沉淀进知识库
...
- **后续**：无（入库完成即结束）
- **特点**：不改 `rules/skills` 配置根，只维护 `.flow-spec/`
...
- **与 `fs-kb-build` 区别**：`ctx-build` 从 `stock-docs` 驱动，`kb-sync` 从会话/代码推断
...
**作用**：将旧版知识库（`docs-index.md` + `rules/` 模式）按主题迁移到 `.flow-spec/` 结构。

**工作原理**：以旧版 `docs-index.md` 和 `rules/main.md(c)` 为「索引线索」，递归识别所有被引用的业务规则和技能文件，按主题粒度重新组织到 `.flow-spec/` 的 `topics/stock-docs/req-docs` 三层结构中。迁移完成后落盘 `migration-report.md`（对照表 + 拟删路径），待用户确认后清理旧文件。本质是一次性的结构重组，将分散的规则/文档归并为统一知识库。
...
- **后续**：`fs-kb-upgrade`（**流程 V1** 旧库须先 migrate 再 upgrade；**现行库 V2+**（含 npm v3.x）见 upgrade 技能步骤 0）
- **流程**：
  1. 以 `docs-index.md` + `rules/main.md(c)` 为主索引
  2. 全量处理业务 `rules/` 与业务 `skills/`（排除 `fs-`* 包技能）
  3. 全量迁移 `stock-docs`/`req-docs`
  4. 落盘 `.flow-spec/migration-report.md`
  5. 用户确认后删除已迁旧的文件
...
**工作原理**：通过「版本分流 + init 代跑」实现升级——先检测当前知识库属于 V1（旧结构，需先 migrate）还是 V2+（已有 `.flow-spec`），V1 走 migrate 后再 init，V2+ 直接代跑 `flow-spec init` 进行包级结构增量对齐（补齐新模板、升级 manifest schema、对齐 matchers 分片格式）。升级后重读 SKILL.md 判断是否需要重跑某些步骤。与单独 `init` 的区别：`kb-upgrade` 包含版本判断和重跑逻辑，`init` 仅做一次性结构补齐。

**使用场景**：

- flow-spec 包版本升级后，升级项目知识库模板
- 旧项目升级到最新结构
- 项目使用 `locale` 模板语言时，默认沿用 `flow-spec.config.json.locale`；未配置则按 `zh-CN` 补齐
- CLI 在交互式 `flow-spec version` / `flow-spec init` 中发现 npm 有新版本后，会提示先 `flow-spec update`，再在 Agent 对话中执行本技能
...
- **前置**：`fs-kb-migrate`（V1 流程）或已存在的 `.flow-spec/`
- **包含**：内部会调用 `flow-spec init` 进行结构对齐
- **注意**：单独的 `flow-spec init` **不是**升级命令

**流程差异**（技能内分流代号，**不等于** npm 包主版本号）：

- **V1**：先 `fs-kb-migrate` 再代跑 `flow-spec init`
- **现行库（V2+）**：已稳定 `.flow-spec` + `manifest-routing` 时，代跑 `flow-spec init` 以对齐 manifest-routing + matchers 分片（**含 flow-spec npm v3.x 等**，详见 `skills/fs-kb-upgrade/SKILL.md` 步骤 0）

**子 agent 调用**：

- `subAgent: false`（默认）：主 agent 内完成升级
- `subAgent: true`：子 agent 仅承接 shell 命令执行（代跑 `flow-spec init`），不承担知识库正文落盘；以下步骤主 agent 不可下放：版本分流（V1 / 现行库 V2+）、init 后重读 SKILL.md 并判断是否整技能重跑、步骤 3b index.md 融合、校验摘要输出
...
| 子 agent | 仅代跑 `flow-spec init` 等 shell 命令，不落盘知识库内容                                      |
...
**工作原理**：以「方案即合约」为核心约束——Agent 以 `req-docs/` 中的技术方案为唯一编码依据，按「理解方案→输出任务列表→提问确认→逐步实现→输出待完成列表」的强制六步流水线执行。任务列表和实现前提问是不可跳过的门禁，确保不会在理解偏差的情况下动手编码。与 `fs-req-plan` 的区别：本规则是轻量单线程编码驱动，不强制创建 `.task/` 追踪（除非 `changeTracking.implement: true`）。
...
- **前置**：`.flow-spec/req-docs/<技术方案>.md`（通过 `fs-req-tech` 或手动放置）
- **规则位置**：
...
通过项目根 `flow-spec.config.json` 控制。默认值：`subAgent=false`、`switchAgentVerification=false`、`changeTracking.feat=true`、`changeTracking.fix=false`、`changeTracking.implement=true`。

### 多端如何「看到」配置（与下文字段表配合）

`subAgent` 等写在 **磁盘 JSON**；各产品不保证自动打开文件，故用 **Cursor 规则 / Claude SessionStart 摘要 + PreToolUse 守门 / Codex SessionStart 摘要 + AGENTS 字段语义表 / 知识库 `config-precheck` 摘要** 多层提示，**权威仍为 Read(`flow-spec.config.json`)**（设计意图见 [设计说明 § 四、5.1](./设计说明.md)，演讲口径见 [flow-spec-演讲稿 Slide 13b](../presentations/flow-spec-intro-public/演讲稿.md)）。**完整路径与表格只维护一处**：[使用说明 § 一、`fs-`* 与 `flow-spec.config.json](./使用说明.md)`。
...
| 子项          | 对应技能                        | 效果                      |
| ----------- | --------------------------- | ----------------------- |
| `feat`      | `fs-kb-feat`               | 执行前创建任务清单，完成后归档，支持跨会话续作 |
| `fix`       | `fs-kb-fix`                | 同上                      |
| `implement` | `fs-implement-tech-design` | 同上                      |


> `fs-req-plan` 不受此配置约束，始终创建任务清单。旧版布尔值（`"changeTracking": true/false`）向下兼容，自动展开为三项全开/全关。
