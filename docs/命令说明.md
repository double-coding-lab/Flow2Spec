...
**工作原理**：先用 `flow2spec version` / `flow2spec update --check` 分开判断 Core 与 Template，再检测当前知识库属于 V1（旧结构）还是 V2+（已有 `.Knowledge`）。Core-only 更新只刷新 Core 与 Hook；Template 更新才代跑 init，并按 `projectRev` / `pkgRev` 选择快速路径或完整流程。V1 不再内置迁移支持。

**使用场景**：

- Template Version 升级后，对齐项目知识库模板
- 旧项目升级到最新结构
- 项目使用 `locale` 模板语言时，默认沿用 `flow2spec.config.json.locale`；未配置则按 `zh-CN` 补齐
- Hook 检测到 `templateUpdateAvailable=true` 后，在 Agent 对话中执行本技能；Core-only 更新不进入本技能
