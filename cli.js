...
function updateCheckCacheFile() {
  const safeName = String(pkg.name || "flow-spec").replace(/[^a-z0-9_.-]+/gi, "_");
  return path.join(os.homedir(), ".flow-spec", `${safeName}-update-check.json`);
}
...
function shouldCheckForUpdates() {
  if (process.env.FLOW_SPEC_SKIP_UPDATE_CHECK === "1") return false;
...
/**
 * init 收尾时自动把全局 flow-spec 升到 latest。
 *
 * 触发条件（同时满足）：
...
↻ 检测到全局 ${pkg.name}@${installed}，正在升级到 v${latest}...`);
  try {
    ...
    console.error(`
⚠ 全局升级失败：${e.message || e}
  可手动执行：  flow-spec update`);
  }
}

function printKnowledgeUpgradeHint(latest) {
  console.log(`
⚠ flow-spec 有新版本 v${latest}（当前 v${pkg.version}）
建议先更新包：
  flow-spec update
...
用于对齐项目知识库模板、manifest/matchers 与配置根产物；不要把单独 flow-spec init 当作知识库升级。
`);
}
...
const help = `
flow-spec - 统一知识库工作流（AI 配置入口）  v${pkg.version}

用法:
  flow-spec init [agent ...] [--reset-knowledge] [--yes] [--locale zh-CN|en-US]    在当前项目初始化：写入 .flow-spec 与所选 agent 入口
  flow-spec config              打印项目根 ${CONFIG_FILENAME} 的解析结果（缺省值合并后）
  flow-spec version             显示当前 flow-spec 版本
  flow-spec update              更新 flow-spec 到最新版本；更新后提示执行 fs-kb-upgrade
  flow-spec --help              显示本说明

agent（可多个，空格分隔；省略时交互选择）：
  ${agentList}

示例:
  flow-spec init                  # 交互选择工具和配置
  flow-spec init claude           # 直接写入 .claude/，跳过工具选择
  flow-spec init cursor claude    # 同时写入 .cursor/ 与 .claude/
  flow-spec init codex --locale en-US  # 使用英文模板初始化 Codex
  flow-spec init --yes            # 跳过所有问答，使用默认值（适合 CI）
  flow-spec init --reset-knowledge  # 强制用模板覆盖 .flow-spec（谨慎）
...
init 会:
  ...
  3. 默认仅补齐 .flow-spec 缺失模板，并对路由清单做包级/结构增量对齐（manifest-routing + matcherPath 分片；关键词仅写在 matchers/*.json）；不替代 fs-* 对业务文档与路由内容的写入。
     传 --reset-knowledge 时才会强制用模板覆盖 .flow-spec 中模板承载部分。
  4. 在各 agent 配置根写入 rules、skills（Claude 规则自动转 .md；Codex 在仓库根写入完整 AGENTS.md，.codex/ 写入指针）。
     Claude 额外写入 .claude/hooks/fs-config-session.js、fs-config-inject.js 与 .claude/settings.json：
     SessionStart 注入一次配置摘要；PreToolUse 仅在调用 fs-* Skill 时提示首步必须 Read 配置。
     Cursor 额外写入 fs-config-check.mdc（alwaysApply），强制在技能首步读取配置文件；
  ...
  5. 每次 init 将当前 locale 包模板 knowledge/index.md 复制到 .flow-spec/template/index.template.md，供 fs-kb-upgrade 技能与 .flow-spec/index.md 对照；不自动改写 index.md。（「知识库升级」指 fs-kb-upgrade 技能，init 本身不是升级命令。）
  6. 规则与技能在各 agent 配置根加载；其他模版类文件在 .flow-spec/template/ 等目录。
...
if (sub === "version" || sub === "--version" || sub === "-v") {
  console.log(`flow-spec v${pkg.version}`);
...
if (sub === "config") {
  ...
  try {
    const cfg = loadFlowSpecConfig(cwd);
      ...
      const lines = ids.map((id) => {
        ...
        if (id === "claude") {
          const hookLine = claudeHooksResult?.settingsChanged
            ? "rules/、skills/、hooks/fs-config-session.js、hooks/fs-config-inject.js、settings.json（已写入 fs SessionStart/PreToolUse hooks）"
            : "rules/、skills/（settings.json 中 fs hook 已存在，跳过）";
          return `  - ${root}/：（${label}）${hookLine}`;
        }
        return `  - ${root}/：（${label}）rules/、skills/`;
      });
      const knowledgeLine = overwriteKnowledge
        ? "  - .flow-spec/：已按 --reset-knowledge 强制覆盖模板"
        : `  - .flow-spec/：保留已有内容，补齐缺失模板（新增 ${knowledgeResult?.written || 0}，跳过 ${knowledgeResult?.skipped || 0}）`;
      const routingLine = overwriteKnowledge
        ? "  - .flow-spec/manifest-routing.json + .flow-spec/matchers/*：已随 reset 覆盖到模板版本（不再写入 manifest-matchers.json）"
        : routingUpgrade?.upgraded
          ? "  - 路由清单已与模板增量对齐"
          : "  - 路由清单已是最新能力路由，无需变更";
      const indexLine =
        indexSnapshot?.written === false
          ? `  - .flow-spec/template/index.template.md：未复制（${indexSnapshot?.reason || "skip"}）`
          : `  - .flow-spec/template/index.template.md：已从包内 templates/${locale}/knowledge/index.md 复制（与 .flow-spec/index.md 对照见 fs-kb-upgrade 技能）`;
...
建议阅读 README 或 docs/使用说明.md，按「规则在配置根、文档在 .flow-spec」的方式使用。
`);
