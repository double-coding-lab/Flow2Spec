---
description: flow-spec topic-authoring guidelines: topic naming / skeleton / topicMetadata / topicDependencies decisions / whether a rule needs a corresponding topic / disk-write ownership pointers
alwaysApply: false
---

# flow-spec Topic Authoring Guidelines

This rule is the single source of truth for the **authoring side**. Whenever an `fs-*` skill adds or modifies `.flow-spec/topics/<topic>.md`, adjusts `manifest-routing.topicMetadata` / `manifest-routing.topicDependencies`, or deletes / migrates a topic, it **must Read this full rule first**, then continue with the corresponding SKILL steps. It coexists with `fs-flow-spec-unified-entry` (the consumption side); in hard conflicts, the unified entry wins.

## Scope

This rule is touched when any of the following is true:

- Add or rewrite `.flow-spec/topics/<topic>.md`;
...
- Delete or migrate a topic (`fs-kb-rm` / `fs-kb-migrate` / `fs-kb-upgrade`).

## 1. Topic Naming

- **id**: `kebab-case`, matching the key in `manifest-routing.topicPaths`.
- **Filename**: `.flow-spec/topics/<topic-id>.md`. If the topic is strongly bound to an `fs-*` skill / rule of the same name (for example `fs-task` / `fs-req-plan`), the filename may include the `fs-` prefix to show shared origin.
...
**Directory boundary for long-form background references (hard rule)**: in a topic, reference slots that point at long-form sources — sections titled "Detailed background / Related materials / Long-form source / Reference documents" — **may only** point at `.flow-spec/stock-docs/*_终稿.md` or already-finalized `stock-docs/*`. **Do not** put `.flow-spec/req-docs/*` (clarifications / technical designs / SQL / PRDs) in these slots: `req-docs` are **temporary inputs** for a given delivery — they get archived or migrated once the delivery lands, so using them as a topic's long-form source of truth leaves a dangling reference. If the corresponding `.flow-spec/stock-docs/*_终稿.md` does not yet exist when syncing / creating a topic, **first trigger `fs-doc-final`** to consolidate the stock-doc (or confirm with the user and hand-write it), and only then point the topic at that stock-doc; you may not skip stock-doc consolidation and mount the topic on `req-docs`. **Allowed**: the topic body may **briefly cite** a single sentence or a field name from a design document as evidence (e.g., "see `.flow-spec/req-docs/xxx_技术方案.md`" as an occasional inline pointer), but the **long-form background reference slot** (the whole "Detailed background / Related materials" section) still must point at a stock-doc.
...
4. **Dependency declaration** (if dependencies exist in `topicDependencies`, the body must explicitly state "before executing, read dependency topic `<dep>` first"; use the first paragraph of `topics/fs-req-plan.md` as a reference);
5. **Boundaries and prohibited items** (avoid expanding into neighboring topics);
6. **Long-form background / detailed materials reference** (when the topic needs to carry business background): list only clickable Markdown links to `.flow-spec/stock-docs/*_终稿.md` (1–3 links); **do not** list `.flow-spec/req-docs/*` here. If no stock-doc has been consolidated yet, **generate the stock-doc first** and then fill this section.
...
1. **Strong reference to a prerequisite rule**: A's execution steps **explicitly mention** B's terminology / artifacts / disk-write constraints (example: `fs-req-plan` requires maintaining `.task/` according to `fs-task`).
...
3. **Shared disk-write target**: A and B write the same set of files and B defines the disk-write format (such as `.task/` or `.flow-spec/topics/`).
...
- during a `fs-kb-upgrade` audit, the same topic is repeatedly matched by several unrelated task types.
...
- **Yes** (user questions / inputs can trigger this rule's execution) -> create a corresponding routing summary in `.flow-spec/topics/` and configure an entry in `taskToTopicRules`. Examples: `fs-task` (matched by change-tracking user scenarios), `fs-implement-tech-design` ("implement from design" user scenario).
- **No** (only referenced internally by other rules / SKILLs, and users will not initiate it directly) -> **do not create** a topic. Examples: `fs-knowledge-preflight`, `fs-karpathy-guidelines`, `fs-config-check`, this rule `fs-topic-authoring`.
...
Write-ownership constraints for `manifest-routing.json` / `.flow-spec/index.md` / `.flow-spec/topics/*.md` **are governed by `fs-flow-spec-unified-entry` and the "hard write-ownership constraints" inside each SKILL**. This rule does not repeat them; in conflicts, follow the unified entry and the corresponding SKILL.
...
- **Listing `.flow-spec/req-docs/*` files (clarifications / technical designs / SQL / PRDs) in a topic's "Long-form background / Detailed materials / Related materials / Long-form source / Reference documents" reference slot.** These slots may only point at `.flow-spec/stock-docs/*_终稿.md`; when no stock-doc exists yet, consolidate it first and then fill the slot. Short-sentence / inline evidence pointers are not covered by this prohibition.
