...
description: >
  ...
  The corresponding skill takes effect only when at least one of changeTracking.feat / fix / implement in flow-spec.config.json is true.
  Trigger words: changeTracking, task tracking, change tracking, continue work, resume previous task; 任务追踪、变更追踪、续作、继续上次任务
alwaysApply: true
---

# fs-task (Change-Tracking Rule)

## Effective Conditions

Each skill checks its own subfield:

- `fs-kb-feat`: read `changeTracking.feat`
- `fs-kb-fix`: read `changeTracking.fix`
- `fs-implement-tech-design`: read `changeTracking.implement`
...
> The `fs-req-plan` command is not constrained by this condition and always runs (see `skills/fs-req-plan/SKILL.md`).

## Multi-developer collaboration and `TASK_ROOT` (resolve first)

Before any read/write under `.task`, **must** `Read("flow-spec.config.json")` and resolve **`TASK_ROOT`** (fixed for the session; do not change id mid-session):
...
5. **`.flow-spec/` stays shared**; this rule does not per-developer the knowledge base.

> Implementation reference: package `lib/developerId.js` (`resolveDeveloperContext` / `taskRootFor`).

## Binding When fs-req-plan Is Invoked

When executing **`fs-req-plan`** (or continuing a task matched by `linkedSkill: "fs-req-plan"`):

- It is **not constrained** by `changeTracking.feat` / `fix` / `implement`, but **must** maintain the task tree under **`TASK_ROOT`** per this rule.
- Skill **step 0** must `Read` this full rule (**Cursor/Claude**: `rules/fs-task.*`; **Codex**: `.codex/fs-rules/fs-task.md`).
...
- If this session created a **`git worktree`** or equivalent isolated directory for a subtask: before ending, follow **`fs-flow-spec-unified-entry`** "Git worktree and subtask working-directory hygiene" to remove it or record the leftover path and deletion command (write it to `user-todos.md` when needed).
...
## Related Materials
- `.flow-spec/req-docs/<capability>-spec.md`
- `.flow-spec/stock-docs/<capability>-arch.md`
...
1. **When creating a task** (`fs-task` "Task start" step 3.e): create this file; it may contain a short note plus an empty list.
...
- [ ] Execute in the target environment: `.flow-spec/req-docs/xxx.sql` (back up first)
...
**Scope of effect**: generated for any task that uses `.task/` — both automatic mode (`changeTracking.feat` / `fix` / `implement`) and explicit mode (`fs-req-plan`); not skill-specific.

**Disk-write obligation**:

1. **When creating a task** (after `fs-task` "Task start" step 3.e): `acceptance.md` **may** be created at the same time with a placeholder note (e.g. "After every `task.md` item is `[x]`, the Agent fills in the acceptance checklist here"). **Do not** prewrite acceptance items before implementation; that would risk drifting from the final delivery.
...
- [ ] `.flow-spec/topics/<topic>.md`: <added/revised description> (verification: open the file to confirm sections are complete)
- [ ] `.flow-spec/manifest-routing.json`: <whether changed and why> (verification: read the corresponding field)
...
- Do not create a `.task/` directory when all of `changeTracking.feat` / `changeTracking.fix` / `changeTracking.implement` are `false` or missing (`fs-req-plan` is not constrained by this).
