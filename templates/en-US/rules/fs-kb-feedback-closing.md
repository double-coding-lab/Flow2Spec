---
description: Closing rule for knowledge-base follow-up suggestions after source code was read during ordinary Q&A; only suggest fs-kb-distill, do not write automatically
alwaysApply: true
---

# flow-spec Knowledge-Base Feedback Closing
...
- This turn has **not entered** an `fs-*` skill, `implement-tech-design`, `fs-git-commit`, or another existing follow-up flow;
...
1. **KB does not cover it + source code provided the answer**: append this at the end of the answer:
   ```md
   > 💡 Run `fs-kb-distill` to ingest knowledge from this turn
   >
...
- **case 1** (`fs-kb-distill` uncovered scenario): no topic covers this capability / module / problem domain
...
- **case 2** (`fs-kb-distill` supplement scenario): an existing topic covers the direction but lacks details
...
- Cases 1-3: output one Markdown blockquote containing, in order, the `fs-kb-distill` command + one blank line + the **This turn will ingest** summary (one line, see "Summary requirement" above).
...
- Only suggest; do not automatically run `fs-kb-distill`.
