...
> **Single long-form rule**: this file is the complete execution rule for **implement-tech-design**. `.flow-spec/topics/fs-implement-tech-design.md` is only a routing summary; **Codex** reads `.codex/fs-rules/fs-implement-tech-design.md` (automatically mirrored from this file by `flow-spec init`) as the equivalent rule text.

> Execution scope: the unified knowledge-base path is `/.flow-spec/`. All paths below are interpreted according to the `.flow-spec` convention.

# Implement Deliverables From a Technical Design (General)

When the user asks to implement a runnable deliverable based on a **technical design document** (the user provides a document path such as `.flow-spec/req-docs/xxx.md`, or a PDF), follow these conventions.

**Directory convention**: `.flow-spec/req-docs/` stores technical designs "used for implementation"; `.flow-spec/stock-docs/` stores consolidated documents and must not be used as direct coding input.
...
- If the user provides a PDF: first run `fs-doc-pdf`, convert the PDF to MD under `.flow-spec/req-docs/`, then continue.
...
- PDF input: first run `fs-doc-pdf` and obtain `.flow-spec/req-docs/*.md`.
...
2. Read project conventions such as README, `.flow-spec/stock-docs/`, architecture notes, and existing modules to align implementation style.
...
If `changeTracking.implement: true`, after outputting the task list, write this checklist to `.task/active/<task-name>/task.md` according to the `fs-task` rule.

### Step 2.6: Sync Change Tracking With `task.md` / `user-todos.md` (Only When `changeTracking.implement: true`)

- Whenever work corresponding to an implementation task-list item is completed, use `Edit` **in the same session** to update the corresponding `[ ]` -> `[x]` in `.task/active/<task-name>/task.md`. Do not defer this to closing, and do not replace disk updates with verbal completion claims (see `fs-task` "During execution" and "Interruption and session end").
- Whenever an item appears during execution that **must be done by the user** (database changes, environment configuration, etc.), append it **in the same session** to `.task/active/<task-name>/user-todos.md` (see `fs-task` "user-todos.md").
...
- If the user wants to consolidate knowledge into the KB, remind them that `fs-kb-build` can later sync topics and routing.
...
- If `changeTracking.implement: true`: do not skip step 2.6 (write back `task.md` checkboxes as implementation progresses and append `user-todos.md`); archiving must satisfy the `fs-task` archive gate.
