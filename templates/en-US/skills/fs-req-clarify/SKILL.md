---
name: fs-req-clarify
description: Clarify a PRD or requirement through follow-up questions until it is actionable, then use fs-req-tech to produce a technical design; triggers: 需求澄清、PRD 澄清、requirement clarification、PRD clarification
---
...
> Execution scope: clarification documents are written to `.flow-spec/req-docs/`.

**Input**: Optional. The full PRD, a requirement description, or a document path (for example `.flow-spec/req-docs/xxx.md`). If omitted, clarify based on the current conversation. Later replies may add requirement conditions.
...
**Prohibited**:
- Appending an `fs-kb-distill` closing hint at the end of the clarification document or immediately after it (see the prohibited section of `rules/fs-kb-feedback-closing.*` — process-orchestration skills do not trigger distill on write);
- Auto-chaining to `fs-req-tech` before the clarification document is written to disk (the auto-chain requires an on-disk clarification path);
- Skip-chaining to `fs-req-plan` / `implement-tech-design` / any other `fs-*` skill in the same turn (only one hop — to `fs-req-tech` — is allowed; subsequent skills still require a new user turn).
