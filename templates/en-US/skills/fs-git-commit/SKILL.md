...
- The semantics of `subAgent` / `switchAgentVerification` use the unified entry as the only source of truth: **Cursor/Claude** read the config-root `rules/fs-flow-spec-unified-entry.*`; **Codex** reads `.codex/fs-rules/fs-flow-spec-unified-entry.md`.
...
- Skip **Step 2: Knowledge-base coverage check**. Do not read `.flow-spec/topics/` / `.flow-spec/stock-docs/` for coverage judgment.
- Do not prompt the user to run `fs-kb-sync` / `fs-kb-feat` first.
...
**First check whether `.flow-spec/` exists:**

- If `.flow-spec/manifest-routing.json` does not exist: skip this step, mention in Step 5 that "the project has not initialized the flow-spec knowledge base; consider running flow-spec init", and continue to Step 3.
...
2. Read the directory lists of `.flow-spec/topics/` and `.flow-spec/stock-docs/`.
...
Recommended before committing:
  A) Run fs-kb-sync now to record them, then automatically continue the commit flow
  B) Commit first and record them manually later (enter B to confirm)
  C) Cancel this commit (enter C)
```

- Choose **A**: prompt the user to run `fs-kb-sync` or `fs-kb-feat`. After the user finishes recording and says so in the **same session**, or triggers this skill again, continue from Step 1 or Step 3 (**do not require** a separate "continue commit" confirmation; this matches Steps 3-4).
...
📌 Reminder: the following capabilities are still not in the knowledge base; record them before merging:
  - <capability description>
  You can run: fs-kb-sync or fs-kb-feat

[If Step 2 was skipped because .flow-spec does not exist]
💡 This project has not initialized the flow-spec knowledge base. To enable it, run: flow-spec init
...
4. Was the knowledge-base check performed or skipped with an explicit reason (quick commit / `.flow-spec` missing)? Must be yes.
