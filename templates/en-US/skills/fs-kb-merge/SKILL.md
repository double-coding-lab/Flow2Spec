---
name: fs-kb-merge
description: Resolve editor-context conflicts after a Git merge; optionally accept conflict files; implementation-side conflicts are only summarized for user confirmation; triggers: 合并上下文冲突、fs-kb-merge、merge context conflicts、resolve kb merge
---
...
# Resolve Context Merge Conflicts (fs-kb-merge)
...
- **`/修正实现规则` (`fs-kb-fix`)**: targeted correction and documentation/rule sync after the user has identified a problem.
