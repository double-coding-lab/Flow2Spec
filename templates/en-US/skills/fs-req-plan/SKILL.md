---
name: fs-req-plan
description: Plan and implement tasks from a technical design, requirement description, or change description; always maintain `.task/` according to fs-task; supports parallel sub-agent implementation. Triggers: fs-req-plan、创建任务、任务规划、我需要任务清单、task planning、create task list
---
...
# Requirement Task Planning and Implementation (fs-req-plan)

Start from a requirement or technical design and cover the full "plan -> implement" chain. This skill **does not depend on** `changeTracking.*`, but the full `.task/` lifecycle **must use `fs-task` as the only source of truth** (directory structure, format, continuation, checkbox updates, archiving, and user-todos). Knowledge-base sync is invoked later by the user as needed through `fs-kb-feat` / `fs-kb-sync`.

## Relationship with fs-task (Hard Constraint)

| Item | Description |
| --- | --- |
| **Source of truth** | Config-root **`rules/fs-task.*`** (`alwaysApply: true`); Codex reads **`.codex/fs-rules/fs-task.md`** (init mirror, same source as rules) |
...
| **Relationship with changeTracking** | `fs-req-plan` is **not constrained by** `changeTracking.feat/fix/implement`; it **always** uses task lists. See `fs-task` "Activation Conditions" |

**All three clients must read the full `fs-task` text (Step 0 is mandatory, before any step below):**

| Client | Path |
| --- | --- |
| **Cursor** | Config-root `rules/fs-task.mdc`; or initialized `.cursor/rules/fs-task.mdc` |
| **Claude Code** | `.claude/rules/fs-task.md` |
| **Codex** | `.codex/fs-rules/fs-task.md` |

## Orchestration (main / sub agent)

- `subAgent` / `switchAgentVerification` use the unified entry as the only source of truth: **Cursor/Claude** -> `rules/fs-flow-spec-unified-entry.*`; **Codex** -> `.codex/fs-rules/fs-flow-spec-unified-entry.md`.
- **Step 1 (continuation triage + parsing)**: the main agent must perform `fs-task` "Task Start" 1-2. Document parsing may be split to a sub agent (read-only).
...
- **Step 3 (write task files)**: follow `fs-task` "Task Start" 3.a-3.f. `todo.json` is **main-agent only**. Drafts of `task.md` / `context.md` / `user-todos.md` may be created by a sub agent; additions to `user-todos.md` during execution are merged by the main agent.
...
- **Step 5 (archive)**: main agent only. Execute only after the archive gates in `fs-task` "Task Completion" are satisfied.
- Worktree hygiene follows `fs-flow-spec-unified-entry`; interruption/end-of-session handling follows `fs-task` "Interruption and Session End".

## Input (Choose One)

- Technical design path (`.flow-spec/req-docs/*.md` or PDF)
...
1. **`Read("flow-spec.config.json")`** (project root; missing fields are treated as `false`).
2. **`Read` the full `fs-task` text for one of the three clients above** (do not skip; do not use only this SKILL summary as a substitute).
...
#### 1a. Continuation Triage (`fs-task` "Task Start" 1-2, Main Agent)
...
2. **Exactly 1 match** -> `Read` the corresponding `task.md` and `context.md`; if present, `Read` **`user-todos.md`**. Show the remaining checklist and unchecked user todos, then ask whether to **continue** that task.
   - User confirms continuation -> **load the full text of this SKILL** (`linkedSkill` should be `fs-req-plan`), continue from the first `[ ]` in `task.md`, and **do not** create a duplicate `active/` directory. **Jump to Step 4** (if planning still needs additions, record them first under `## Notes`, then implement).
...
4. **No match** -> check for **orphan `active/`** tasks (`fs-task`): if any unarchived task has a `task.md` containing `[ ]`, ask whether to continue it or restore `todo.json`; otherwise proceed to 1b.
...
- Read `.flow-spec/stock-docs/` and other context for alignment.
- Convert PDFs to MD first with `fs-doc-pdf`.
...
### Step 3: Write Task Files (`fs-task` "Task Start" 3.a-3.f)

After the user confirms, **strictly follow `fs-task`** (the format is defined by that rule body; do not omit files):
...
| 3.d | Write **`context.md`**: touched files, `.flow-spec` links; user todos point to `user-todos.md` | Main or sub |
| 3.e | Create **`user-todos.md`** (fixed filename; when there are no todos, write a placeholder note) | Main or sub |
| 3.f | Add a **`todo.json` entry**: `name`, `folder`, `keywords` (including Step 2 suggestions), `linkedSkill: "fs-req-plan"`, `createdAt` | **Main agent only** |
...
Follow `fs-task` "In Progress" and "Interruption and Session End":
...
### Step 5: Archive the Task (`fs-task` "Task Completion")
...
```markdown
## fs-req-plan complete: <task-name>
...
### Todo (Knowledge Base)
- You may later call fs-kb-sync / fs-kb-feat
...
- **Step 0**: must first `Read` `flow-spec.config.json` + the **full `fs-task` text** (three-client paths above).
- **`.task/`**: always obey `fs-task`; this SKILL must not conflict with it.
...
1. Has the **full `fs-task` text** been read, and do written files match its format?
