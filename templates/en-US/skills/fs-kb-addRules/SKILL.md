---
name: fs-kb-addRules
description: Capture user-spoken rules into the knowledge base, automatically decide "create new topic / merge into existing topic", and sync routing; does not write code or create `.task/`; triggers: fs-kb-addRules、新增规则、口述规则、把这条记到知识库、add rule、capture spoken rule
---

> **Task paths**: all `.task/` reads/writes must use **`TASK_ROOT` from `rules/fs-task`** (` .task` or `.task/<developerId>`; config → git → legacy). Bare `.task/todo.json` / `.task/active/` below mean **`TASK_ROOT/...`**.


> Execution scope: this skill only maintains `.flow-spec` (`topics/index/manifest-routing/matchers` shards), does not modify the configuration-root `rules/skills`, does not touch business code, and does not create `.task/` (spoken rules are meta-configuration changes, not business change tracking).

# fs-kb-addRules: Put User-Spoken Rules into the Knowledge Base

## Boundary with Existing Skills

- Different from `fs-kb-feat`: `fs-kb-feat` is strongly tied to "code implementation + KB sync" and creates `.task/` when `changeTracking.feat` is hit; this skill **only captures rules**, does not change code, and does not track tasks.
- Different from `fs-kb-build`: `fs-kb-build` takes `.flow-spec/stock-docs/<file>_final.md` as input; this skill takes **rule text spoken by the user in the current conversation**.
- Different from `fs-kb-add`: `fs-kb-add` aggregates "multiple source/config files" into stock-docs; this skill skips stock-docs and writes directly to topics.

## Orchestration (main / sub-agent)

- The meaning of `subAgent` / `switchAgentVerification` uses the unified entry as the only source of truth (**Cursor/Claude** read `rules/fs-flow-spec-unified-entry.*`; **Codex** reads `.codex/fs-rules/fs-flow-spec-unified-entry.md`). This SKILL does not repeat it.
...
- **Write-authority hard rule**: `.flow-spec/manifest-routing.json` / `.flow-spec/index.md` are always written by the main agent.
...
Before executing any step, **Read** the full `rules/fs-topic-authoring.*` (**Cursor/Claude**: `rules/fs-topic-authoring.mdc`; **Codex**: `.codex/fs-rules/fs-topic-authoring.md`). All later naming, skeleton, dependency judgment, DAG minimization, and write ownership follow that guideline.
...
- Read `.flow-spec/manifest-routing.json` to get the full `topicPaths` set.
- Read the topic table in `.flow-spec/index.md`, scanning once by topic id + one-sentence intent.
...
- **No overlap / low overlap** (no suitable host found) -> propose "**create** `topics/<new-id>.md`"; generate the new id from the rule body in **kebab-case**, following `fs-topic-authoring` naming constraints (no version suffix, no personal nickname, no conflict with existing `index.md` titles).
...
- **Create**: write the five skeleton items from `fs-topic-authoring` section 2 "topic body skeleton" one by one (title and one-sentence intent / applicable scenario / core rules / dependency declaration / boundaries and forbidden items).
...
- Writing style follows `fs-flow-spec-unified-entry` "knowledge-base writing style": **affirmative wording first**; mutually exclusive choices are exceptions.

### 4b. Judge `topicDependencies` (Required)

Use the four questions + reverse exclusion + DAG minimization in `fs-topic-authoring` section 4. Scan newly written text for **backtick references to other topic ids / rule filenames**, and judge each one:
...
- **New topic**:
  - Add `manifest-routing.topicPaths`: `<id> -> .flow-spec/topics/<id>.md`.
  ...
  - Add `taskToTopicRules[]` only when this rule should be matched as **user task routing** (see `fs-topic-authoring` section 5 criteria); internal rules that are only referenced by other rules / SKILLs **do not enter** `taskToTopicRules`.
  - If `taskToTopicRules[]` is added, create `.flow-spec/matchers/<matcherId>.json`, extracting `includeAny` keywords from the user's wording (the original words + 1-2 obvious synonyms; prefer missing terms over over-broad terms).
...
- Target: .flow-spec/topics/<id>.md (section: <optional>)

### Knowledge-Base Changes
- .flow-spec/topics/<id>.md: <new/revision description>
- .flow-spec/manifest-routing.json: <whether topicPaths / taskToTopicRules / topicDependencies were updated and why>
- .flow-spec/matchers/<id>.json: <whether includeAny was updated and why>
- .flow-spec/index.md: <whether updated and why>
...
- Prefer merging into the same topic to avoid creating near-duplicate topics (see the "do not" naming items in `fs-topic-authoring`).
- `manifest-routing.json` and `.flow-spec/index.md` are always written by the main agent (write-authority hard rule).
...
The scan finds existing `topics/fs-git-commit.md` (describes the git commit flow).
- Step 3 proposes: **merge into** the "commit style" section of `topics/fs-git-commit.md`.
...
This clearly involves two topics: `fs-implement-tech-design` (implementation discipline) and `fs-git-commit` (submission flow).
...
1. The full `rules/fs-topic-authoring.*` was Read before writing.
