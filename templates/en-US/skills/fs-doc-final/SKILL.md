---
name: fs-doc-final
description: Convert a PDF or MD document into the `final-overview-template` standard format so fs-kb-build can later sync topics/index/manifest; triggers: fs-doc-final、转成概述模板、终稿模版、final-overview-template, final template、convert to final draft
---

> Execution scope: drafts and final documents are both written to `.flow-spec/stock-docs/`; prefer reading `.flow-spec/template/final-overview-template.md` as the template.
...
The user provides **at least one argument** after this skill: the **first argument** is a local **PDF file path** or **Markdown file path** (required); the **second argument** (optional) is an output file path and overrides the default output location. Execute the following workflow based on the file type, and output a final-style Markdown document that can later be consumed by **fs-kb-build**.

**The `final-overview-template` is only guidance**: if `.flow-spec/template/final-overview-template.md` exists, read it as a structural reference. Do not force an exact template fit.

## Embedded Template Structure (Use When `.flow-spec/template/final-overview-template.md` Does Not Exist)
...
2. **Reference format** (not mandatory): if `.flow-spec/template/final-overview-template.md` exists, read it as structural guidance; otherwise use the embedded template structure below.
...
4. **Output**:
  - Default output is `.flow-spec/stock-docs/<design-name>_final.md` (the final artifact includes the `_final` marker).
  - If the user specifies an output path as the second argument, use that path; otherwise use the default.
5. **Reply**: tell the user that `.flow-spec/stock-docs/<design-name>_final.md` has been generated, and say they may continue with `fs-kb-build` to sync `.flow-spec/topics`, `.flow-spec/index.md`, and `manifest` if needed.
...
1. **Try to read the PDF**: read the PDF from the user-provided path (absolute or relative to the project root, for example `.flow-spec/stock-docs/xxx.pdf`).
  ...
  - If the PDF cannot be read directly (for example only binary data is available): ask the user to export the PDF content to `.flow-spec/stock-docs/xxx.md` and then run the skill again.
2. **Generate the draft**:
  - Save the extracted content as `.flow-spec/stock-docs/<design-name>_draft.md` (infer the design name from the PDF filename or first heading).
  - In the reply, **show the full draft or its main structure**, and clearly state:
    - "The draft has been saved as `.flow-spec/stock-docs/<design-name>_draft.md`; please review and edit it."
    - "After confirming it is correct, run: `fs-doc-final .flow-spec/stock-docs/<design-name>_draft.md`."
...
When the user **runs this skill again with the draft `.md` path** (for example `.flow-spec/stock-docs/技术方案设计_draft.md`):
...
- **Output suggestion**: generate `.flow-spec/stock-docs/<design-name>_final.md`.
- **Reply**: tell the user the standardized version has been generated, and say they may continue with `fs-kb-build` to sync `.flow-spec/topics` and the index.

---

## Path and Output Conventions

- All paths are relative to the project root. Drafts and final documents are both placed under `.flow-spec/stock-docs/`.
- **Input**: the first argument is a required file path, such as `.flow-spec/stock-docs/方案.pdf` or `.flow-spec/stock-docs/方案_draft.md`; the second argument is optional.
- **Output**:
  - First PDF run: `.flow-spec/stock-docs/<design-name>_draft.md`
  - MD or draft MD: `.flow-spec/stock-docs/<design-name>_final.md`
- If `.flow-spec/stock-docs/` does not exist, create it before writing.
...
- End with a one-sentence summary: the generated draft/final path and the next step, `fs-kb-build`, for syncing knowledge routing topics and indexes.
