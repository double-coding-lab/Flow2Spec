---
name: fs-doc-arch
description: Generate a first draft of project architecture documentation from user notes, documents, or code scanning; no fixed format is required as long as the explanation is clear. Triggers: 项目架构说明、fs-doc-arch、架构初稿、architecture draft、project architecture
---
> Execution scope: this skill writes its artifact to `.flow-spec/stock-docs/` by default; later knowledge-base skill chains such as `fs-doc-final` and `fs-kb-build` sync it into `.flow-spec/topics/index/manifest`.

## Orchestration (main / sub agent)

- The semantics of `subAgent` / `switchAgentVerification` use the unified entry as the only source of truth: **Cursor/Claude** read the config-root `rules/fs-flow-spec-unified-entry.*`; **Codex** reads `.codex/fs-rules/fs-flow-spec-unified-entry.md` (same source, mirrored by `flow-spec init`). This section does not restate those semantics.
...
- **Write-authority hard constraint**: `.flow-spec/index.md` / `manifest-routing.json` are always written by the main agent. Sub agents must not touch them.
...
**Division of responsibility with fs-kb-add**: this skill is responsible **only** for the "architecture documentation **draft**" step. By default, it does **not** write the final version in the same skill and does not directly run **fs-kb-build**. If the user wants to parse an **already completed capability** into the knowledge base **in one pass** from multiple related file paths (draft -> final -> topics/index/manifest), use **`fs-kb-add`**. **Do not use this skill to impersonate that workflow**.
...
| -------------- | -------------------------- |
| **First argument** | Optional. One of: **a plain-text description** written after the command, or **a local document path** such as `.flow-spec/stock-docs/xxx.md`, `.flow-spec/req-docs/README.md`, or `README.md`. If omitted, enter the "no input" flow. |
| **Second argument** | Optional. Output file path. If omitted, default to `.flow-spec/stock-docs/architecture-overview_draft.md` (the project name may be inferred from `package.json` `name` or the directory name, then sanitized for a valid filename). |
...
4. **Output**
   - Default output: `.flow-spec/stock-docs/architecture-overview_draft.md`. If the user provides a second argument, write to that path.
...
3. **Output**
   - Same as above: default `.flow-spec/stock-docs/architecture-overview_draft.md`, or the second argument specified by the user.
...
Do not chain "overview -> details" through topicDependencies (see fs-topic-authoring section 5).
```

The user may choose: **A) run `fs-doc-arch` separately for each split recommendation** (recommended), or **B) continue with the current single draft** for later steps.

## Next Step After Completion (Hard Constraint)

This skill **only produces a draft**. At the end, guide the user in the following order. **Do not** let the user skip the final version and directly run `fs-kb-build`:

1. Tell the user the draft path and recommend reviewing and completing it first.
2. **The next step must be `fs-doc-final`**: use the draft path as input and produce `.flow-spec/stock-docs/<design-name>_final.md` in the `final-overview-template` standard format.
3. **Only after the final document is written** guide the user to **`fs-kb-build`**, and its input must be the final path (containing `_final` or just generated by `fs-doc-final`).
4. **Do not** write only "please run `fs-kb-build`" in the completion reply with input pointing to `*_draft.md`; **do not** present `fs-kb-build` and `fs-doc-final` as alternatives.
5. **Only exception**: the user **explicitly requests** skipping the final step, and the draft has already been manually made compliant with the `final-overview-template`. First explain the risk of skipping finalization, then allow `fs-kb-build`.

**Completion reply template** (must include both `fs-doc-final` and `fs-kb-build`, with ctx-build after the final document):

> Architecture draft generated: `<draft-path>`. Please review and edit it first; next run **`fs-doc-final <draft-path>`** to convert it to a final document, then run **`fs-kb-build <final-path>`** to sync knowledge routing topics and indexes.
...
- **Default output**: `.flow-spec/stock-docs/architecture-overview_draft.md`; the project name is taken from `package.json` `name` (with scope and illegal characters removed) or the current directory name.
...
- Summarize according to the "completion reply template" above: draft path + **must run `fs-doc-final` before `fs-kb-build`**; do not recommend build only.
