---
name: fs-kb-distill
description: Extract reusable knowledge facts from Q&A and auto-commit to KB; decide whether to create new topic or append to existing topic based on drill-down depth; trigger: fs-kb-distill, extract knowledge from Q&A, distill knowledge from conversation
---
...
> Execution scope: This skill only maintains `.flow-spec`, does not modify config root `rules/skills` by default.

## Orchestration (main / sub agent)

- `subAgent` / `switchAgentVerification` semantics follow unified entry as single source of truth: **Cursor/Claude** read config root `rules/fs-flow-spec-unified-entry.*`; **Codex** read `.codex/fs-rules/fs-flow-spec-unified-entry.md` (same source, mirrored by `flow-spec init`).
...
- Write permission constraint: `manifest-routing.json` and `.flow-spec/index.md` are always written by main agent only.
- Verification: self-verify on the side that writes to disk.

# fs-kb-distill: Q&A-Driven Knowledge Extraction and Ingestion

## When to Use

- User asks → agent drills down source code to answer → need to solidify discovered knowledge into KB
- Usually auto-suggested by `fs-kb-feedback-closing` rule, can also be manually invoked by user
- Distinction from `fs-kb-sync`: `sync` is for batch syncing multiple capabilities; `distill` focuses on knowledge extraction from single Q&A
...
| Matched topic | Optional | If triggered by `fs-kb-feedback-closing`, carries matched topicId |
...
`fs-kb-distill` has **only one** entry (no `--fast` parameter). The first thing on entering this flow is to **decide the tier**:

### Tier judgment (4 dimensions; light tier only when all 4 are satisfied)

| Dimension | How to measure | Condition for **light tier** |
| --- | --- | --- |
| Upstream `fs-kb-feedback-closing` case | Look at the closing block at the end of this/last agent reply | **case 2 or case 3** (case 1 / no closing block → strict tier) |
...
> **Business source defined**: a Read whose path is **not** under `.claude/` / `.cursor/` / `.codex/` / `.flow-spec/` / `.task/` counts. Rule files / topics / config / task lists do not count.
...
1. Read `flow-spec.config.json` (get `subAgent` / `switchAgentVerification`)
2. Read `rules/fs-kb-feedback-closing.mdc` (get "reusable knowledge facts" definition)
3. Read `rules/fs-topic-authoring.mdc` (get topic authoring guidelines)
...
3. **Matched topic**: Extract matched topicId if `fs-kb-feedback-closing` already analyzed; otherwise re-route based on question
...
Extract following types of knowledge from answer (refer to `fs-kb-feedback-closing`):
...
> ****Light tier** skips the entire decision matrix**: adopt the upstream `fs-kb-feedback-closing` "this round will ingest: <summary>" conclusion directly:
...
> **Authoring guidelines**: This step triggers creation/modification of topics and possible `topicDependencies`, must follow already-read `fs-topic-authoring` guidelines.
...
- If creating new topic:
  - Add new entry in `.flow-spec/index.md`
...
1. If has stock-doc: write to `.flow-spec/stock-docs/<doc>.md`
2. Write or update `.flow-spec/topics/<topicId>.md`
3. Update `.flow-spec/manifest-routing.json`
4. Update `.flow-spec/matchers/<id>.json`
5. Update `.flow-spec/index.md`
...
### Modified Files
- .flow-spec/topics/<topicId>.md: <modification description>
- .flow-spec/index.md: <modification description or "unchanged">
- .flow-spec/manifest-routing.json: <modification description or "unchanged">
- .flow-spec/matchers/<id>.json: <modification description or "unchanged">
- .flow-spec/stock-docs/<doc>.md: <modification description or "unchanged">
...
- Only maintain `.flow-spec`, do not modify config root `rules/skills`
...
2. Extracted all "reusable knowledge facts" (refer to `fs-kb-feedback-closing` definition)
...
8. The reply does NOT append any of `fs-kb-feedback-closing`'s case 1–4 closing blocks at the end (must be yes; this skill itself is a knowledge-base write, and by the reverse prohibition in `fs-kb-feedback-closing`'s "Applicable Scope", pasting a distill hint here is forbidden).
