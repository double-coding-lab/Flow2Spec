---
name: fs-kb-build
description: Generate knowledge-routing topics and indexes from `.flow-spec/stock-docs` documents; triggers: 生成项目上下文、fs-kb-build、终稿生成上下文、generate project context、build knowledge context
---

> Execution scope: this skill only maintains `.flow-spec` (`topics/index/manifest-routing/matchers` shards) and does not modify the configuration-root `rules/skills`. It no longer maintains `.flow-spec/manifest-matchers.json` (deprecated aggregate file; `flow-spec init` deletes legacy copies).

# Generate Project Context from Documents (topics/index/routing manifest)

## Orchestration (main / sub-agent)

- The meaning of `subAgent` / `switchAgentVerification` uses the unified entry as the only source of truth: **Cursor/Claude** read the configuration-root `rules/fs-flow-spec-unified-entry.*`; **Codex** reads `.codex/fs-rules/fs-flow-spec-unified-entry.md` (same source, mirrored by `flow-spec init`). This SKILL does not repeat those definitions.
...
- **Medium/large change branch** (`subAgent=true` and above threshold):
  - The main agent lists a **file-level contract** in the main session: sub-agent A only writes `.flow-spec/topics/<foo>.md`, sub-agent B only writes `.flow-spec/matchers/<m-foo>.json`, and paths do not overlap.
  - Sub-agents only write files inside their contract and do not cross boundaries.
  - The **main agent alone** edits `.flow-spec/manifest-routing.json` / `.flow-spec/index.md` (adding `taskToTopicRules`, `topicPaths`, `matcherPath`, `topicDependencies`, `topicMetadata`).
...
- **Write-authority hard rule**: `.flow-spec/manifest-routing.json` (including `topicMetadata`) / `.flow-spec/index.md` are **always written by the main agent**; sub-agents must not touch them.
...
- Local paths must be under `.flow-spec/stock-docs/`.
- **Must be a final draft**: recommended filename contains `_final.md`, or has been normalized by **`fs-doc-final`**. It is **forbidden** to execute this skill directly with a `*_draft.md` produced by `fs-doc-arch`.
- If the input path contains **`_draft`**, or the user has just completed an architecture draft but has not run `fs-doc-final`: **stop** and reply that they must first run **`fs-doc-final <draft-path>`**, then call this skill with the final-draft path after it is written.
- If `.flow-spec/req-docs/` is passed, tell the user to organize it into a `stock-docs` final draft before executing.
...
- URL: fetch the body; if inaccessible, ask the user to first save it as `.flow-spec/stock-docs/*.md`.
...
> **Authoring-side guideline**: this step involves adding/modifying topics and `topicDependencies`, so first Read the full `rules/fs-topic-authoring.*` (**Cursor/Claude**: `rules/fs-topic-authoring.mdc`; **Codex**: `.codex/fs-rules/fs-topic-authoring.md`) before continuing to step 3 / step 5. Naming, skeleton, dependency judgment, DAG minimization, and judgment timing all follow that guideline; this SKILL does not repeat them.
...
- Target path: `.flow-spec/topics/<topic>.md`
...
- Update the topic routing table in `.flow-spec/index.md`.
...
- `sourceDoc` or document references uniformly point to `.flow-spec/stock-docs/<filename>.md`.
- Do not use `.flow-spec/req-docs/` as a topic `sourceDoc`.
...
User input: `fs-kb-build .flow-spec/stock-docs/<capability>_final.md`, and an existing `topics/<capability>.md` already exists.
...
1. `.flow-spec/topics/*.md` and `manifest-routing.topicPaths` correspond one-to-one.
...
7. `manifest-routing.json` / `.flow-spec/index.md` were written at a single point by the main agent, with no unauthorized sub-agent writes.
