---
name: fs-doc-milestone
description: Generate a milestone document (`project-milestone-template`) from req-docs, git log, `.task`, and knowledge-topic semantics; triggers: fs-doc-milestone、生成项目里程碑、里程碑、project milestone、generate milestone. A semantic scope may be appended after the command. This skill always uses a sub agent for generation and the main agent for verification, regardless of flow-spec.config orchestration switches
---
...
> Execution scope: read `.flow-spec/template/project-milestone-template.md`; write **only** `.flow-spec/stock-docs/<scope-name>-milestones.md` (no second path argument).

## Orchestration (Fixed, Not Affected by Project Config)

**This skill is not affected by** `subAgent`, `switchAgentVerification` (or old key `subAgentVerification`) in `flow-spec.config.json`: regardless of whether they are `true` or `false`, **always** use the division below. It is **forbidden** to switch to "all main session" or "sub agent self-verifies and ends" because of config values.
...
> Step 0 still **`Read("flow-spec.config.json")`** (satisfies the `fs-config-check` preflight), but its `subAgent` / `switchAgentVerification` values **must not** change this skill's orchestration.
...
| `template` | `.flow-spec/template/project-milestone-template.md` (do not write the template's top explanatory blockquote) |
...
| --- | --- | --- |
| **req-docs** | In-scope `.flow-spec/req-docs/*.md` | Requirement/design nodes, delivery summaries |
...
1. **`Read(".flow-spec/manifest-routing.json")`**: extract `topicPaths`, `taskToTopicRules`, and scope-related `topicDependencies`.
2. **`Read(".flow-spec/index.md")`**: at least the "**Topic Overview**" table (topic id, applicable scenario, linked document summary).
3. **Read `.flow-spec/topics/<topic>.md` as needed**: summaries related to the scope or manifest hits (**do not** enumerate the entire `topics/` directory; read only topics named by manifest/index, usually no more than the table rows).
...
1. **`Read("flow-spec.config.json")`** (do not use its `subAgent` / `switchAgentVerification` values to orchestrate this skill)
2. **`Read(".flow-spec/template/project-milestone-template.md")`**
...
5. **Similar-file check (required before writing)**: list existing `*milestone*.md` files under `.flow-spec/stock-docs/` (including `*milestone.md`). If there is a file with the **same target path** or a **semantically similar** file (for example, another whole-project milestone such as `project-milestones.md`, or high overlap in prefix/scope keywords), **ask the user first**. Do **not** silently overwrite or invent a new filename:
