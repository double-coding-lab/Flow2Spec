...
'use strict';
/**
 * flow-spec PreToolUse guard — only reminds before invoking an fs-* Skill that flow-spec.config.json must be Read first.
 ...
 * Written by flow-spec init --claude to .claude/hooks/fs-config-inject.js.
 */
...
process.stdin.on('end', () => {
  ...
  if (!/^fs-/.test(skillName)) {
    process.exit(0);
    return;
  }

  emitAdditionalContext([
    `[flow-spec] About to invoke ${skillName}. Before entering that Skill body, the first action must be Read("flow-spec.config.json").`,
