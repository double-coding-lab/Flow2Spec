...
'use strict';
/**
 * flow-spec SessionStart hook — checks for version updates on the first conversation each day.
 ...
 *   - Behind -> inject one notice into Agent context (suggest running fs-kb-upgrade)
 ...
 * Written by flow-spec init to the corresponding agent's hooks/fs-update-check.js.
 */
const fs   = require('fs');
const path = require('path');
const { execFileSync } = require('child_process');

const MANIFEST_PATH = path.join(process.cwd(), '.flow-spec', 'manifest-routing.json');
const CACHE_DIR    = path.join(process.cwd(), '.flow-spec');
const CACHE_FILE   = path.join(CACHE_DIR, 'update-check.json');
const PACKAGE_NAME_PLACEHOLDER = '__FLOW_SPEC_' + 'PACKAGE_NAME__';
const PACKAGE_NAME = '__FLOW_SPEC_PACKAGE_NAME__';
...
function buildNotice(latestNpm, manifestVersion) {
  const userMsg = `📦 [flow-spec] The current project "${getProjectName()}" knowledge-base version is v${manifestVersion}, which is lower than the latest package version v${latestNpm}. You can run the fs-kb-upgrade skill to align templates and routing.`;
  return [
    `[flow-spec/agent-instruction] At the start of this response, you must relay the following upgrade notice to the user verbatim. Do not omit it or skip it after only internal judgment:`,
    ``,
    userMsg,
  ].join('\n');
}
...
function isEnabled() {
  try {
    const cfg = JSON.parse(fs.readFileSync(
      path.join(process.cwd(), 'flow-spec.config.json'), 'utf8'
    ));
