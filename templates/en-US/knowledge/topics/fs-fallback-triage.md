...
> - Yes -> routing terms may be missing; consider running `fs-kb-build` / `fs-kb-sync` to supplement routing, then retry
...
> - Unsure -> please check whether `.flow-spec/stock-docs/` contains related documents, then tell me
...
| User confirms docs exist but routing terms are missing | Suggest running `fs-kb-build` / `fs-kb-sync` to supplement routing; pause this run or drill into source code |
