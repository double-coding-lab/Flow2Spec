...
description: >
  ...
  仅当 flow-spec.config.json 中 changeTracking.feat / fix / implement 之一为 true 时，对应技能生效。
...
- `fs-kb-feat`：读 `changeTracking.feat`
- `fs-kb-fix`：读 `changeTracking.fix`
- `fs-implement-tech-design`：读 `changeTracking.implement`

若对应子项为 `false` 或字段不存在，**该技能内的变更追踪步骤不执行**，直接跳过。

> `fs-req-plan` 命令不受此条件约束，始终执行（见 `skills/fs-req-plan/SKILL.md`）。

## 多人协作与任务根 `TASK_ROOT`（必须先解析）

进入本规则任何「读/写 `.task`」步骤前，**必须** `Read("flow-spec.config.json")`，并解析 **`TASK_ROOT`**（本会话内固定，禁止中途改 id）：
...
- 若本会话为子任务创建过 **`git worktree`** 或等价隔离目录：结束前按 **`fs-flow-spec-unified-entry`**「Git worktree 与子任务工作目录卫生」完成移除或写明残留路径与删除命令（必要时写入 `user-todos.md`）。
...
## 相关资料
- `.flow-spec/req-docs/<能力>-spec.md`
- `.flow-spec/stock-docs/<能力>-arch.md`
...
1. **创建任务时**（`fs-task`「任务开始」步骤 3.e）：创建该文件；可含简短说明 + 空列表。
...
- [ ] 在目标环境执行：`.flow-spec/req-docs/xxx.sql`（先备份）
...
**生效范围**：凡使用 `.task/` 的任务均生成（自动模式 `changeTracking.feat` / `fix` / `implement` 命中、以及显式模式 `fs-req-plan`）；不区分技能。

**写盘义务**：

1. **创建任务时**（`fs-task`「任务开始」步骤 3.e 之后）：**可同时创建** `acceptance.md` 并写占位说明（如「task.md 全部 `[x]` 后由 Agent 在此填入验收清单」）；尚未实现时**不得**预先写入验收点，避免与最终交付脱节。
...
- [ ] `.flow-spec/topics/<topic>.md`：<新增/修订说明>（验收方式：打开该文件确认章节齐全）
- [ ] `.flow-spec/manifest-routing.json`：<是否变更与原因>（验收方式：阅读对应字段）
...
在项目 `.claude/settings.json` 中添加，每次文件变更前将活跃任务注入上下文（示例为 **legacy** 单根；多人请改为当前 `TASK_ROOT/todo.json`，或在命令内按 `flow-spec.config.json` + git 解析路径）：
...
- 禁止在 `changeTracking.feat` / `changeTracking.fix` / `changeTracking.implement` 均为 `false` 或字段不存在时创建任务目录（`fs-req-plan` 不受此约束）
