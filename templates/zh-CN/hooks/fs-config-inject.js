...
'use strict';
/**
 * flow-spec PreToolUse guard — 仅在调用 fs-* Skill 前提示必须先 Read flow-spec.config.json。
 * 不在 PreToolUse 中反复注入完整配置；配置摘要由 SessionStart hook 一次性提供。
 * 由 flow-spec init --claude 写入 .claude/hooks/fs-config-inject.js。
 */
...
process.stdin.on('end', () => {
  ...
  emitAdditionalContext([
    `[flow-spec] 即将调用 ${skillName}。进入该 Skill 正文前，首个动作必须 Read("flow-spec.config.json")。`,
