...
'use strict';
/**
 * flow-spec SessionStart hook — 每天第一次对话时检查版本更新。
 * 比较本地知识库 manifest-routing.json 的 version 与 npm 最新版本：
 *   - 一致或本地更新 → 静默退出
 *   - 落后 → 向 Agent 上下文注入一行提示（建议执行 fs-kb-upgrade）
 * 已检查过且无需升级则静默；已检查且仍需升级时，每个新会话继续注入提醒。
 * 由 flow-spec init 写入对应 agent 的 hooks/fs-update-check.js。
 */
const fs   = require('fs');
...
function buildNotice(latestNpm, manifestVersion) {
  const userMsg = `📦 [flow-spec] 当前项目「${getProjectName()}」知识库版本 v${manifestVersion}，低于最新包版本 v${latestNpm}。可执行 fs-kb-upgrade skill 对齐模板与路由。`;
  return [
    `[flow-spec/agent-instruction] 你必须在本轮回复开头将下列升级提示原文转告用户，不得省略、不得仅在内部判断后跳过：`,
    ``,
    userMsg,
  ].join('\n');
}
