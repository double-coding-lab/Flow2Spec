---
name: fs-req-plan
description: 根据技术方案/需求描述/变更描述规划并实现任务；始终按 fs-task 维护 .task/；支持子 agent 并行实现；触发：fs-req-plan、创建任务、任务规划、我需要任务清单
---
...
# 需求任务规划与实现（fs-req-plan）

从需求/技术方案出发，完整覆盖「规划 → 实现」链路。**不依赖** `changeTracking.*`，但 **`.task/` 全生命周期必须以 `fs-task` 为唯一真值源**（目录、格式、续作、打钩、归档、user-todos）。知识库同步由用户后续按需调用 `fs-kb-feat` / `fs-kb-sync`。

## 与 fs-task 的关系（硬约束）

| 项 | 说明 |
| --- | --- |
| **真值源** | 配置根 **`rules/fs-task.*`**（`alwaysApply: true`）；Codex 读 **`.codex/fs-rules/fs-task.md`**（init 镜像，与 rules 同源） |
| **本技能职责** | 规划草稿、实现代码、子 agent 编排；**不得**自定 `.task/` 结构或弱化打钩/归档 |
| **与 changeTracking** | `fs-req-plan` **不受** `changeTracking.feat/fix/implement` 约束，**始终**走任务清单；见 `fs-task`「生效条件」 |

**三端读取 `fs-task` 全文（步骤 0 必做，先于下文任何步骤）**：

| 端 | 路径 |
| --- | --- |
| **Cursor** | 配置根 `rules/fs-task.mdc`；或已 init 的 `.cursor/rules/fs-task.mdc` |
| **Claude Code** | `.claude/rules/fs-task.md` |
| **Codex** | `.codex/fs-rules/fs-task.md` |

## 编排（主 / 子 agent）

- `subAgent` / `switchAgentVerification` 以统一入口为唯一事实源：**Cursor/Claude** → `rules/fs-flow-spec-unified-entry.*`；**Codex** → `.codex/fs-rules/fs-flow-spec-unified-entry.md`。
- **步骤 1（续作分诊 + 解析）**：主 agent 必做 `fs-task`「任务开始」1–2；解析文档可拆子 agent（只读）。
- **步骤 2（草稿确认）**：必须主 agent；未确认前禁止创建 `.task/` 或写业务代码。
- **步骤 3（落盘）**：按 `fs-task`「任务开始」3.a–3.f；`todo.json` **仅主 agent**；`task.md` / `context.md` / `user-todos.md` 初稿可子 agent，`user-todos.md` 执行中追加由主 agent 合并。
- **步骤 4（实现）**：子 agent 只写业务代码；**禁止**子 agent 写 `todo.json`、改 `task.md` checkbox；打钩由主 agent 在合并后当步完成。
- **步骤 5（归档）**：主 agent；**仅**满足 `fs-task`「任务完成」归档门禁后执行。
- worktree 卫生见 `fs-flow-spec-unified-entry`；中断/结束见 `fs-task`「中断与会话结束」。

## 输入（任选其一）

- 技术方案路径（`.flow-spec/req-docs/*.md` 或 PDF）
- 需求 / 变更描述（自由文本）
...
1. **`Read("flow-spec.config.json")`**（项目根；缺失字段视为 `false`）。
2. **`Read` 上表三端之一的 `fs-task` 全文**（不得跳过；不得仅用本 SKILL 摘要代替）。
3. 按读到的 `subAgent` / `switchAgentVerification` 决定下文是否拆子 agent、是否交叉校验。

### 步骤 1：续作分诊 + 解析输入

#### 1a. 续作分诊（`fs-task`「任务开始」1–2，主 agent）
...
2. **命中 1 个** → `Read` 对应 `task.md`、`context.md`；若存在则 `Read` **`user-todos.md`**；向用户展示剩余 checklist 与未勾用户代办；询问是否**续作**该任务。
   - 用户确认续作 → **加载本 SKILL 全文**（`linkedSkill` 应为 `fs-req-plan`），从 `task.md` 首个 `[ ]` 继续；**禁止**新建重复 `active/` 目录；**跳至步骤 4**（若仍需补充规划，先在「## 备注」记录后再实现）。
   - 用户明确要**新任务** → 进入 1b。
3. **命中多个** → 列出候选，让用户选择续作哪一个或新建。
4. **无命中** → 检查**孤儿 `active/`**（`fs-task`）：若有未归档且含 `[ ]` 的 `task.md`，提示是否续作或恢复 `todo.json`；否则进入 1b。
5. **无 `todo.json`** → 进入 1b。
...
- 读取 `.flow-spec/stock-docs/` 等对齐上下文
- PDF 先 `fs-doc-pdf` 转 MD
...
### 步骤 3：落盘任务清单（`fs-task`「任务开始」3.a–3.f）

用户确认后，**严格按 `fs-task` 执行**（格式以该规则正文为准，不得省略文件）：
...
| 3.d | 写入 **`context.md`**：涉及文件、`.flow-spec` 资料链接；用户代办指向 `user-todos.md` | 主或子 |
| 3.e | 创建 **`user-todos.md`**（固定文件名；无代办时写占位说明） | 主或子 |
| 3.f | **`todo.json` 新增条目**：`name`、`folder`、`keywords`（含步骤 2 建议词）、`linkedSkill: "fs-req-plan"`、`createdAt` | **仅主 agent** |

**禁止**：只建 `task.md` 不写 `todo.json`；省略 `user-todos.md`；使用 `completed/<task-name>-<date>` 旧式归档名。

### 步骤 4：实现代码

遵守 `fs-task`「**执行中**」「**中断与会话结束**」：
...
- **步骤 0**：必须先 `Read` `flow-spec.config.json` + **`fs-task` 全文**（三端路径见上表）
- **`.task/`**：一律服从 `fs-task`；本 SKILL 不得与之冲突
