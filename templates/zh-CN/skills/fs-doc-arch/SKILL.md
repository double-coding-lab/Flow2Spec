---
name: fs-doc-arch
description: 根据用户说明或文档（或扫描代码）生成项目架构说明初稿，无固定格式，描述清楚即可；触发：项目架构说明、fs-doc-arch、架构初稿
---
> 执行口径：本技能产物默认写入 `.flow-spec/stock-docs/`，后续由知识库技能链（如 `fs-doc-final`、`fs-kb-build`）同步到 `.flow-spec/topics/index/manifest`。

## 编排（主 / 子 agent）

- `subAgent` / `switchAgentVerification` 两字段语义以统一入口为唯一事实源：**Cursor/Claude** 读配置根 `rules/fs-flow-spec-unified-entry.*`；**Codex** 读 `.codex/fs-rules/fs-flow-spec-unified-entry.md`（与上同源，`flow-spec init` 镜像）。本节不复述。
...
- **写权硬约束**：`.flow-spec/index.md` / `manifest-routing.json` 恒由主 agent 落盘，子 agent 不得触碰。
...
**与 fs-kb-add 的分工**：本技能**只**负责「架构说明类**初稿**」这一环，默认**不**在同一技能内写终稿、不直接执行 **fs-kb-build**。若用户在工作中要把**已做好的能力**依据多份相关文件路径**一次**解析进知识库（初稿→终稿→topics/index/manifest），应使用 **`fs-kb-add`**，**勿用本技能冒充该流程**。
...
| 参数           | 说明                                                                                                                                                              |
| -------------- | -------------------------- |
| **第一个参数** | 可选。可为以下之一：**一段纯文字说明**（直接写在命令后）、**本地文档路径**（如 `.flow-spec/stock-docs/xxx.md`、`.flow-spec/req-docs/README.md`、`README.md`）。不传则进入「无输入」流程。 |
| **第二个参数** | 可选。输出文件路径；若不传，默认写入 `.flow-spec/stock-docs/架构说明_初稿.md`（项目名可从 package.json 的 name 或目录名推断，做合法文件名处理）。 |
...
4. **输出**
   - 默认写入 `.flow-spec/stock-docs/架构说明_初稿.md`；若用户传入第二参数则写入该路径。
   - 若目录不存在则先创建。
...
3. **输出**
   - 同上，默认 `.flow-spec/stock-docs/架构说明_初稿.md`，或用户指定的第二参数。
...
不通过 topicDependencies 串联"概述 → 详情"（见 fs-topic-authoring 第 5 节）。
```

用户可选择：**A) 按拆分建议分别执行 `fs-doc-arch`**（推荐），或 **B) 继续用当前单份初稿**进入后续流程。

## 完成后的下一步（硬约束）

本技能**只产出初稿**；结束时须按下列顺序引导，**禁止**让用户跳过终稿直接 `fs-kb-build`：

1. 告知初稿路径，建议用户先审阅、补充内容。
2. **下一步必须为 `fs-doc-final`**：以初稿路径为入参，产出 `.flow-spec/stock-docs/<方案名>_终稿.md`（《终稿模版》规范格式）。
3. **仅在终稿落盘后**再引导 **`fs-kb-build`**，且入参须为终稿路径（含 `_终稿` 或由 `fs-doc-final` 刚生成）。
4. **禁止**在完成回复中单独写「请执行 `fs-kb-build`」且入参指向 `*_初稿.md`；**禁止**将 `fs-kb-build` 与 `fs-doc-final` 并列成「二选一」。
5. **唯一例外**：用户**明确要求**跳过终稿、且初稿已人工符合终稿模版——须先说明跳过终稿的风险，再允许指向 `fs-kb-build`。

**完成回复模板**（须同时包含 `fs-doc-final` 与 `fs-kb-build`，且 ctx-build 在终稿之后）：

> 已生成架构说明初稿：`<初稿路径>`。请先审阅修改；下一步请执行 **`fs-doc-final <初稿路径>`** 转为终稿，再执行 **`fs-kb-build <终稿路径>`** 同步知识路由主题与索引。
...
- 所有路径均相对于**配置根的父目录**。
- **默认输出**：`.flow-spec/stock-docs/架构说明_初稿.md`；项目名取自 `package.json` 的 `name`（去掉 scope 与非法字符）或当前目录名。
...
- 完成后按上文「完成回复模板」总结：初稿路径 + **必须先 `fs-doc-final` 再 `fs-kb-build`**；不得仅推荐 build。
