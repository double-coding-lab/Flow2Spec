---
name: fs-kb-distill
description: 从问答过程中提取可复用知识事实并自动入库；根据下钻深度与命中主题判断新增主题或补充既有主题；触发：fs-kb-distill、问答知识提取、从对话中提取知识
---
...
- 写权硬约束：`manifest-routing.json` 与 `.flow-spec/index.md` 恒由主 agent 单点落盘。
- 校验：落盘侧自验。

# fs-kb-distill：问答驱动的知识提取与入库

## 使用时机

- 用户提问 → agent 下钻源码回答 → 需要将发现的知识沉淀到 KB
- 通常由 `fs-kb-feedback-closing` 规则自动建议，也可用户主动调用
- 与 `fs-kb-sync` 区分：`sync` 适合批量同步多个能力；`distill` 专注单次问答的知识提取
...
| 命中主题 | 可选 | 如由 `fs-kb-feedback-closing` 触发，会携带命中的 topicId |
...
`fs-kb-distill` 只有**一个**入口（无 `--fast` 参数），进入流程第一件事是**判挡**：

### 判挡依据（4 个维度，全满足才走轻量挡）

| 维度 | 取值方式 | 走「轻量挡」的条件 |
| --- | --- | --- |
| 上游 `fs-kb-feedback-closing` case | 看本轮 / 上一轮 agent 回答末尾的收口块 | **case 2 或 case 3**（case 1 / 无收口 → 严格挡） |
...
> **业务源码定义**：路径**不在** `.claude/` / `.cursor/` / `.codex/` / `.flow-spec/` / `.task/` 这 5 个目录下的 Read 才计数；规则文件 / topic / config / 任务清单都不算。
...
1. 读取 `flow-spec.config.json`（获取 `subAgent` / `switchAgentVerification`）
2. 读取 `.codex/fs-rules/fs-kb-feedback-closing.md`（获取"可复用知识事实"定义）
3. 读取 `.codex/fs-rules/fs-topic-authoring.md`（获取 topic 创作准则）
...
1. **用户问题**：原始问题文本
2. **agent 回答**：完整回答内容
3. **命中主题**：如果 `fs-kb-feedback-closing` 已分析，提取命中的 topicId；否则根据问题重新路由
...
从回答中提取以下类型的知识（参考 `fs-kb-feedback-closing`）：
...
> ****轻量挡**跳过整段决策矩阵**：直接采用上游 `fs-kb-feedback-closing`「本轮将入库：<概要>」给出的结论：
...
> **创作侧准则**：本步骤会触发新增 / 修改 topic 与可能的 `topicDependencies`，须遵循已读取的 `fs-topic-authoring` 准则。
...
1. 如果有 stock-doc：写入 `.flow-spec/stock-docs/<doc>.md`
2. 写入或更新 `.flow-spec/topics/<topicId>.md`
3. 更新 `.flow-spec/manifest-routing.json`
4. 更新 `.flow-spec/matchers/<id>.json`
5. 更新 `.flow-spec/index.md`
...
### 已修改文件
- .flow-spec/topics/<topicId>.md：<修改说明>
- .flow-spec/index.md：<修改说明或"未改动">
...
2. 是否提取了所有"可复用知识事实"（参考 `fs-kb-feedback-closing` 定义）
...
8. 本次回复末尾**没有**追加 `fs-kb-feedback-closing` 的 case 1～4 任何一种收口块（必须为是；本技能就是 distill 入库本身，自指地再贴一遍提示既冗余又会让用户误以为没入库——`fs-kb-feedback-closing`「适用范围」对此有专项禁令）
