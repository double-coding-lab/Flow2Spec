---
name: fs-kb-addRules
description: 把用户口述的规则沉淀进知识库，自动判定「新建主题 / 并入存量主题」并同步路由；不写代码、不创建 .task/；触发：fs-kb-addRules、新增规则、口述规则、把这条记到知识库
---
...
> 执行口径：本技能只维护 `.flow-spec`（`topics/index/manifest-routing/matchers` 分片），不改配置根 `rules/skills`，不动业务代码，不创建 `.task/`（口述规则属于元配置变更，不是业务变更追踪）。

# fs-kb-addRules：用户口述规则进知识库

## 与既有技能的边界

- 与 `fs-kb-feat` 区分：`fs-kb-feat` 强绑「代码实现 + KB 同步」，命中 `changeTracking.feat` 会创建 `.task/`；本技能**只沉淀规则**，不改代码、不追踪任务。
- 与 `fs-kb-build` 区分：`fs-kb-build` 输入是 `.flow-spec/stock-docs/<file>_终稿.md`；本技能输入是**用户当场口述的规则文本**。
- 与 `fs-kb-add` 区分：`fs-kb-add` 输入是「多文件源码 / 配置」聚合到 stock-docs；本技能跳过 stock-docs，直接落 topic。

## 编排（主 / 子 agent）

- `subAgent` / `switchAgentVerification` 语义以统一入口为唯一事实源（**Cursor/Claude** 读 `rules/fs-flow-spec-unified-entry.*`；**Codex** 读 `.codex/fs-rules/fs-flow-spec-unified-entry.md`）。本 SKILL 不复述。
- 默认主 agent 全流程执行——口述规则单条短文，拆子收益低于 context 切换成本。
- **写权硬约束**：`.flow-spec/manifest-routing.json` / `.flow-spec/index.md` 恒由主 agent 落盘。
- 落盘侧自验。
...
执行任何步骤前，**须先 Read** `rules/fs-topic-authoring.*` 全文（**Cursor/Claude**：`rules/fs-topic-authoring.mdc`；**Codex**：`.codex/fs-rules/fs-topic-authoring.md`），后续命名 / 骨架 / 依赖判定 / DAG 最小化 / 写盘权属均以该条为准。
...
- Read `.flow-spec/manifest-routing.json` 取 `topicPaths` 全集；
- Read `.flow-spec/index.md` 主题表，按主题 id + 一句话意图扫一遍；
...
- **无重合 / 低重合**（找不到合适宿主）→ 提议「**新建** `topics/<新 id>.md`」；新 id 由本技能按规则正文生成 **kebab-case**，遵循 `fs-topic-authoring` 命名约束（无版本后缀、无个人花名、与 `index.md` 既有标题不冲突）。
...
- **新建**：按 `fs-topic-authoring` 第 2 节"topic 正文骨架"五点逐项写入（标题与一句话意图 / 适用场景 / 核心规则 / 依赖声明 / 边界与禁止项）；
- **并入**：在用户确认的章节 / 段落处**手术式插入**——只增加与本次规则直接相关的句段，禁止整文件重写或借机重述背景；
- 行文遵守 `fs-flow-spec-unified-entry`「知识库落盘文风」**肯定式优先**；排他性选择例外。

### 4b. `topicDependencies` 判定（必须）

按 `fs-topic-authoring` 第 4 节四问 + 反向排除 + DAG 最小化，扫新写正文中**反引号引用的其他 topic id / 规则文件名**，逐个判定是否声明依赖：
...
- **新建主题**：
  - 补 `manifest-routing.topicPaths`：`<id> -> .flow-spec/topics/<id>.md`；
  ...
  - 视情况补 `taskToTopicRules[]`——**仅当**该规则会作为**用户任务路由命中**（参见 `fs-topic-authoring` 第 5 节判据）才补；纯被其它规则 / SKILL 引用的内部规则**不进** `taskToTopicRules`；
  - 若补了 `taskToTopicRules[]`，须新建 `.flow-spec/matchers/<matcherId>.json`，从用户口述中抽取 `includeAny` 关键词（用户原话 + 1–2 个明显近义说法，宁缺勿滥）；
- **并入存量主题**：
  - `topicPaths` 不变；
...
### 落盘决策
- 模式：新建 / 并入 / 跨主题拆分
- 目标：.flow-spec/topics/<id>.md（章节：<可选>）

### 知识库变更
- .flow-spec/topics/<id>.md：<新增 / 修订说明>
- .flow-spec/manifest-routing.json：<topicPaths / taskToTopicRules / topicDependencies 是否更新与原因>
- .flow-spec/matchers/<id>.json：<是否更新 includeAny 与原因>
- .flow-spec/index.md：<是否更新与原因>
...
- 同主题优先并入，避免新建近似主题（参见 `fs-topic-authoring` 命名"不要"项）。
- `manifest-routing.json` 与 `.flow-spec/index.md` 恒由主 agent 落盘（写权硬约束）。
- 路由清单仅做最小改动，不重写无关字段。
...
扫描发现已存在 `topics/fs-git-commit.md`（描述 git commit 流程）。
- 步骤 3 提议：**并入** `topics/fs-git-commit.md` 的「commit 文风」章节；
...
明显涉及 `fs-implement-tech-design`（实现纪律）和 `fs-git-commit`（提交流程）两个主题。
...
1. 是否在落盘前 Read 了 `rules/fs-topic-authoring.*` 全文。
