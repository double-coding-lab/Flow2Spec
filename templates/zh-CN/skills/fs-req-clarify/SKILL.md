...
**结束（澄清文档落盘 → 自动衔接技术方案）**：当信息已足够清晰时，必须输出一份可直接落盘的「需求澄清文档」（Markdown）。文档至少包含：背景与目标、范围（包含/不包含）、关键流程、边界与异常、关键概念定义、验收标准、未决问题（如有）。建议保存到 `.flow-spec/req-docs/`（推荐命名 `<能力名>_需求澄清.md`）。

**澄清文档落盘后本技能同轮自动衔接 `fs-req-tech`**：以刚落盘的澄清文档路径为输入直接进入技术方案生成，无需等用户再次触发；进入前给用户一行提示「澄清文档已就绪：`<路径>`；正在按 `fs-req-tech` 生成技术方案」，然后继续。
...
**禁止**：
- 在澄清文档尾部或紧随其后追加 `fs-kb-distill` 收口提示（见 `rules/fs-kb-feedback-closing.*` 禁止段——过程编排型技能落盘不触发 distill）；
- 未落盘澄清文档就自动衔接 `fs-req-tech`（自动衔接的前提是磁盘上已有澄清文档路径）；
- 越级自动衔接 `fs-req-plan` / `implement-tech-design` / 其他 `fs-*` 技能（同轮只允许接到 `fs-req-tech` 一步，后续仍须用户新一轮触发）。
