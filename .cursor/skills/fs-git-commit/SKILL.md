---
name: fs-git-commit
description: 代码写完后提交 Git：默认检查变更与知识库覆盖；用户明确要求“快捷提交”时跳过知识库覆盖检查；生成带 emoji 首行的提交说明后**可直接 commit**（须在当条回复展示首行，不要求用户单独确认 commit）；**git pull 类拉取须用户先确认**。触发：fs-git-commit、提交代码、快捷提交、git commit、帮我提交
---
...
- `subAgent` / `switchAgentVerification` 语义以统一入口为唯一事实源：**Cursor/Claude** 读配置根 `rules/fs-flow-spec-unified-entry.*`；**Codex** 读 `.codex/fs-rules/fs-flow-spec-unified-entry.md`。
...
- 跳过 **步骤 2：知识库覆盖检查**，不读取 `.flow-spec/topics/` / `.flow-spec/stock-docs/` 做覆盖判断。
- 不提示用户先运行 `fs-kb-sync` / `fs-kb-feat`。
...
- 若 `.flow-spec/manifest-routing.json` 不存在：跳过本步骤，在步骤 5 收尾提示「项目尚未初始化 flow-spec 知识库，建议运行 flow-spec init」，继续步骤 3。

**存在时执行覆盖检查：**

1. 从 `git diff HEAD` 及 untracked 文件路径推断本次变更涉及的**功能模块**（以仓库内目录/包名为准，勿臆测未出现的业务名）。
2. 读取 `.flow-spec/topics/` 目录列表与 `.flow-spec/stock-docs/` 目录列表。
...
建议在提交前同步知识库，可选：
  A) 现在运行 fs-kb-sync 补录，完成后自动继续提交流程
  B) 先提交，稍后手动补录（输入 B 确认）
  C) 取消本次提交（输入 C）
```

- 选 **A**：提示用户运行 `fs-kb-sync` 或 `fs-kb-feat` 补录；用户补录完成后在**同一会话声明已补录**或**再次触发本技能**时，从步骤 1 或步骤 3 继续（**不要求**为「继续 commit」单独打字确认，与步骤 3–4 一致）。
...
📌 提醒：以下能力仍未入知识库，建议在合并前补录：
  - <能力描述>
  可运行：fs-kb-sync 或 fs-kb-feat

[若跳过了步骤 2（.flow-spec 不存在）]
💡 项目尚未初始化 flow-spec 知识库，如需接入可运行：flow-spec init
...
4. 知识库检查是否执行或有明确跳过理由（快捷提交 / `.flow-spec` 不存在）（必须为是）。
